...
impl<'a> StructuredSheet<'a> for LiveMinigamesSheet {
    type Row = LiveMinigamesRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a LiveMinigamesSheet {
    type Item = (u32, Vec<(u16, LiveMinigamesRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct LiveMinigamesRow<'a> {
    row: &'a Row,
}
impl<'a> LiveMinigamesRow<'a> {
