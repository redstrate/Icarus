...
impl<'a> StructuredSheet<'a> for GcArmyExpeditionTraitSheet {
    type Row = GcArmyExpeditionTraitRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a GcArmyExpeditionTraitSheet {
    type Item = (u32, Vec<(u16, GcArmyExpeditionTraitRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct GcArmyExpeditionTraitRow<'a> {
    row: &'a Row,
}
impl<'a> GcArmyExpeditionTraitRow<'a> {
