...
impl YKWSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "YKW")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "YKW", &exh, language, i)?);
        }
...
impl YKWRow {
    ...
    pub fn Companion<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn Location<'a>(&'a self) -> [&'a ColumnData; 6] {
        [
            &self.columns[3],
            &self.columns[4],
            &self.columns[5],
