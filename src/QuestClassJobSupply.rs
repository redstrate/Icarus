...
impl QuestClassJobSupplySheet {
    ...
    pub fn row(&self, row_id: u32) -> Option<QuestClassJobSupplyRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<QuestClassJobSupplyRow> {
...
impl<'a> StructuredSheet<'a> for QuestClassJobSupplySheet {
    type Row = QuestClassJobSupplyRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a QuestClassJobSupplySheet {
    type Item = (u32, Vec<(u16, QuestClassJobSupplyRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, QuestClassJobSupplySheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, QuestClassJobSupplySheet> {
