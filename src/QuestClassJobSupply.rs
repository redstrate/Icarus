...
impl QuestClassJobSupplySheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "QuestClassJobSupply")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "QuestClassJobSupply", &exh, language, i)?,
                );
        }
...
impl QuestClassJobSupplyRow {
    ...
    pub fn ENpcResident<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    ...
    pub fn AmountRequired<'a>(&'a self) -> &'a ColumnData {
        &self.columns[9]
    }
    pub fn Unknown3<'a>(&'a self) -> &'a ColumnData {
        &self.columns[10]
    }
    pub fn ItemHQ<'a>(&'a self) -> &'a ColumnData {
        &self.columns[11]
    }
}
