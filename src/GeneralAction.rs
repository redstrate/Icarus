...
impl GeneralActionSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "GeneralAction")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "GeneralAction", &exh, language, i)?);
        }
...
impl GeneralActionRow {
    ...
    pub fn Recast<'a>(&'a self) -> &'a ColumnData {
        &self.columns[6]
    }
