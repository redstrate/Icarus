...
impl<'a> StructuredSheet<'a> for MJICraftworksObjectSheet {
    type Row = MJICraftworksObjectRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a MJICraftworksObjectSheet {
    type Item = (u32, Vec<(u16, MJICraftworksObjectRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MJICraftworksObjectRow<'a> {
    row: &'a Row,
}
impl<'a> MJICraftworksObjectRow<'a> {
    pub fn Item(&'a self) -> u16 {
        self.row.columns[0].into_u16().copied().unwrap()
    }
    pub fn Theme(&'a self) -> [u16; 2] {
        [
