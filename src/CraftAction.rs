...
impl CraftActionSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "CraftAction")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "CraftAction", &exh, language, i)?);
        }
...
impl CraftActionRow {
    ...
    pub fn QuestRequirement<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    ...
    pub fn RequiredStatus<'a>(&'a self) -> &'a ColumnData {
        &self.columns[14]
    }
    ...
    pub fn Specialist<'a>(&'a self) -> &'a ColumnData {
        &self.columns[19]
    }
}
