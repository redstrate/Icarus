...
impl<'a> StructuredSheet<'a> for FCReputationSheet {
    type Row = FCReputationRow<'a>;
...
impl<'a> IntoIterator for &'a FCReputationSheet {
    type Item = (u32, Vec<(u16, FCReputationRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct FCReputationRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> FCReputationRow<'a> {
    pub fn Name(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn PointsToNext(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn RequiredPoints(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn Color(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn DiscountRate(&'a self) -> &'a Field {
