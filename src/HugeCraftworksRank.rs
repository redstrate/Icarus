...
impl HugeCraftworksRankSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "HugeCraftworksRank")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "HugeCraftworksRank", &exh, language, i)?,
                );
        }
...
impl HugeCraftworksRankRow {
    pub fn ExpRewardPerItem<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
