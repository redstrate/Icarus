...
impl<'a> StructuredSheet<'a> for QuestLinkMarkerSheet {
    type Row = QuestLinkMarkerRow<'a>;
...
impl<'a> IntoIterator for &'a QuestLinkMarkerSheet {
    type Item = (u32, Vec<(u16, QuestLinkMarkerRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct QuestLinkMarkerRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> QuestLinkMarkerRow<'a> {
    pub fn SourceMap(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn Level(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn TargetMap(&'a self) -> &'a Field {
