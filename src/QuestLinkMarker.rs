...
impl QuestLinkMarkerSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "QuestLinkMarker")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(read_excel_sheet(resource, "QuestLinkMarker", &exh, language, i)?);
        }
...
impl QuestLinkMarkerRow {
    pub fn SourceMap<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn Level<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn TargetMap<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
