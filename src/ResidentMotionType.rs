...
impl<'a> StructuredSheet<'a> for ResidentMotionTypeSheet {
    type Row = ResidentMotionTypeRow<'a>;
...
impl<'a> IntoIterator for &'a ResidentMotionTypeSheet {
    type Item = (u32, Vec<(u16, ResidentMotionTypeRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ResidentMotionTypeRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> ResidentMotionTypeRow<'a> {
