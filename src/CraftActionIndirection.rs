...
impl<'a> StructuredSheet<'a> for CraftActionIndirectionSheet {
    type Row = CraftActionIndirectionRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a CraftActionIndirectionSheet {
    type Item = (u32, Vec<(u16, CraftActionIndirectionRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct CraftActionIndirectionRow<'a> {
    row: &'a Row,
}
impl<'a> CraftActionIndirectionRow<'a> {
