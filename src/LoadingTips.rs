...
impl<'a> StructuredSheet<'a> for LoadingTipsSheet {
    type Row = LoadingTipsRow<'a>;
...
impl<'a> IntoIterator for &'a LoadingTipsSheet {
    type Item = (u32, Vec<(u16, LoadingTipsRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct LoadingTipsRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> LoadingTipsRow<'a> {
