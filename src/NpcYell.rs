...
impl NpcYellSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "NpcYell")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "NpcYell", &exh, language, i)?);
        }
...
impl NpcYellRow {
    ...
    pub fn BalloonTime<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    ...
    pub fn OutputType<'a>(&'a self) -> &'a ColumnData {
        &self.columns[6]
    }
    ...
    pub fn IsBalloonSlow<'a>(&'a self) -> &'a ColumnData {
        &self.columns[13]
    }
    pub fn BattleTalkTime<'a>(&'a self) -> &'a ColumnData {
        &self.columns[14]
    }
}
