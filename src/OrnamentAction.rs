...
impl OrnamentActionSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "OrnamentAction")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "OrnamentAction", &exh, language, i)?);
        }
...
impl OrnamentActionRow {
    pub fn Actions<'a>(&'a self) -> [&'a ColumnData; 6] {
        [
            &self.columns[0],
            &self.columns[1],
            &self.columns[2],
