...
impl GatheringPointSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "GatheringPoint")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "GatheringPoint", &exh, language, i)?);
        }
...
impl GatheringPointRow {
    pub fn GatheringPointBase<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn GatheringPointBonus<'a>(&'a self) -> [&'a ColumnData; 2] {
    ...
    pub fn Count<'a>(&'a self) -> &'a ColumnData {
        &self.columns[8]
    }
