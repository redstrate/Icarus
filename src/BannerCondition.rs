...
impl<'a> StructuredSheet<'a> for BannerConditionSheet {
    type Row = BannerConditionRow<'a>;
...
impl<'a> IntoIterator for &'a BannerConditionSheet {
    type Item = (u32, Vec<(u16, BannerConditionRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct BannerConditionRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> BannerConditionRow<'a> {
    pub fn UnlockCriteria1(&'a self) -> [&'a Field; 2] {
        [
    ...
    pub fn UnlockCriteria3(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn UnlockCriteria4(&'a self) -> &'a Field {
    ...
    pub fn UnlockType1(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[7]]
    }
    pub fn UnlockType2(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[8]]
    }
    pub fn PrerequisiteType(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[9]]
    }
    pub fn UnlockHint(&'a self) -> &'a Field {
