...
impl MKDRelicGrowth2ContentListSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "MKDRelicGrowth2ContentList")?;
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<MKDRelicGrowth2ContentListRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(MKDRelicGrowth2ContentListRow {
            columns,
        })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn get_row(&self, row_id: u32) -> Option<MKDRelicGrowth2ContentListRow> {
