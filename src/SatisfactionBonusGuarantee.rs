...
impl SatisfactionBonusGuaranteeSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "SatisfactionBonusGuarantee")?;
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<SatisfactionBonusGuaranteeRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(SatisfactionBonusGuaranteeRow {
            columns,
        })
    }
...
impl SatisfactionBonusGuaranteeRow {
    pub fn BonusDoH<'a>(&'a self) -> [&'a ColumnData; 2] {
        [&self.columns[0], &self.columns[1]]
    }
    pub fn BonusDoL<'a>(&'a self) -> [&'a ColumnData; 2] {
        [&self.columns[2], &self.columns[3]]
    }
    pub fn BonusFisher<'a>(&'a self) -> [&'a ColumnData; 2] {
