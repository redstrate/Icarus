...
impl FrontlineSheet {
    ...
    pub fn read_from(
        ...
        let exh = resolver.read_excel_sheet_header("Frontline")?;
        let sheet = resolver.read_excel_sheet(&exh, "Frontline", language)?;
        Ok(Self { sheet })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn row(&self, row_id: u32) -> Option<FrontlineRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<FrontlineRow> {
...
impl<'a> StructuredSheet<'a> for FrontlineSheet {
    type Row = FrontlineRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a FrontlineSheet {
    type Item = (u32, Vec<(u16, FrontlineRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, FrontlineSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, FrontlineSheet> {
...
#[derive(Debug, Clone)]
pub struct FrontlineRow<'a> {
    row: &'a Row,
}
impl<'a> FrontlineRow<'a> {
