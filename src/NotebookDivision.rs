...
impl StructuredSheet for NotebookDivisionSheet {
    type Row = NotebookDivisionRow;
...
impl<'a> IntoIterator for &'a NotebookDivisionSheet {
    type Item = (u32, Vec<(u16, NotebookDivisionRow)>);
...
impl NotebookDivisionRow {
    ...
    pub fn QuestUnlock<'a>(&'a self) -> &'a Field {
        &self.columns[1]
    }
    pub fn NotebookDivisionCategory<'a>(&'a self) -> &'a Field {
        &self.columns[2]
    }
    pub fn CraftOpeningLevel<'a>(&'a self) -> &'a Field {
        &self.columns[3]
    }
    pub fn GatheringOpeningLevel<'a>(&'a self) -> &'a Field {
        &self.columns[4]
    }
    pub fn Unknown0<'a>(&'a self) -> &'a Field {
        &self.columns[5]
    }
    pub fn SecretRecipeBookGroups<'a>(&'a self) -> [&'a Field; 2] {
        [&self.columns[6], &self.columns[7]]
    }
    /// Index is CraftType
    pub fn AllowedCraftTypes<'a>(&'a self) -> [&'a Field; 8] {
        [
            &self.columns[8],
            &self.columns[9],
            &self.columns[10],
    ...
    pub fn RequiresSecretRecipeBookGroupUnlock<'a>(&'a self) -> &'a Field {
        &self.columns[16]
    }
}
