//! This file is auto-generated, do not edit it manually! This is generated based on the schema from https://github.com/xivdev/EXDSchema.
#![allow(warnings)]
use crate::{StructuredSheet, StructuredSheetIterator};
use physis::{
    Error, resource::{Resource, ResourceResolver},
    exd::EXD, exh::{EXH, ExcelColumnDefinition},
    excel::{Sheet, Field, Row},
    Language,
};
#[derive(Debug, Clone)]
pub struct NotebookDivisionSheet {
    sheet: Sheet,
}
impl NotebookDivisionSheet {
    /// Read the sheet from a `ResourceResolver`.
    pub fn read_from(
        resolver: &mut ResourceResolver,
        language: Language,
    ) -> Result<Self, Error> {
        let exh = resolver.read_excel_sheet_header("NotebookDivision")?;
        let sheet = resolver.read_excel_sheet(&exh, "NotebookDivision", language)?;
        Ok(Self { sheet })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn row(&self, row_id: u32) -> Option<NotebookDivisionRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<NotebookDivisionRow> {
        let row = &self.sheet.subrow(row_id, subrow_id)?;
        self.read_row(row)
    }
    /// Returns the number of rows in this sheet.
    pub fn row_count(&self) -> u32 {
        self.sheet.exh.header.row_count
    }
}
impl StructuredSheet for NotebookDivisionSheet {
    type Row = NotebookDivisionRow;
    fn read_row(&self, row: &Row) -> Option<Self::Row> {
        Some(Self::Row {
            Name: row
                .columns[0]
                .into_string()
                .cloned()
                .expect("Expected column 0 to be a string!"),
            QuestUnlock: row
                .columns[4]
                .into_u32()
                .copied()
                .expect("Expected column 4 to be a uint32!"),
            NotebookDivisionCategory: row
                .columns[1]
                .into_u8()
                .copied()
                .expect("Expected column 1 to be a uint8!"),
            CraftOpeningLevel: row
                .columns[2]
                .into_u8()
                .copied()
                .expect("Expected column 2 to be a uint8!"),
            GatheringOpeningLevel: row
                .columns[3]
                .into_u8()
                .copied()
                .expect("Expected column 3 to be a uint8!"),
            Unknown1: row
                .columns[5]
                .into_u8()
                .copied()
                .expect("Expected column 5 to be a uint8!"),
            Unknown0: row
                .columns[6]
                .into_u8()
                .copied()
                .expect("Expected column 6 to be a uint8!"),
            SecretRecipeBookGroups: [
                row
                    .columns[8]
                    .into_u8()
                    .copied()
                    .expect("Expected column 8 to be a uint8!"),
                row
                    .columns[9]
                    .into_u8()
                    .copied()
                    .expect("Expected column 9 to be a uint8!"),
            ],
            AllowedCraftTypes: [
                row
                    .columns[10]
                    .into_bool()
                    .copied()
                    .expect("Expected column 10 to be a bool!"),
                row
                    .columns[11]
                    .into_bool()
                    .copied()
                    .expect("Expected column 11 to be a bool!"),
                row
                    .columns[12]
                    .into_bool()
                    .copied()
                    .expect("Expected column 12 to be a bool!"),
                row
                    .columns[13]
                    .into_bool()
                    .copied()
                    .expect("Expected column 13 to be a bool!"),
                row
                    .columns[14]
                    .into_bool()
                    .copied()
                    .expect("Expected column 14 to be a bool!"),
                row
                    .columns[15]
                    .into_bool()
                    .copied()
                    .expect("Expected column 15 to be a bool!"),
                row
                    .columns[16]
                    .into_bool()
                    .copied()
                    .expect("Expected column 16 to be a bool!"),
                row
                    .columns[17]
                    .into_bool()
                    .copied()
                    .expect("Expected column 17 to be a bool!"),
            ],
            RequiresSecretRecipeBookGroupUnlock: row
                .columns[7]
                .into_bool()
                .copied()
                .expect("Expected column 7 to be a bool!"),
        })
    }
}
impl<'a> IntoIterator for &'a NotebookDivisionSheet {
    type Item = (u32, Vec<(u16, NotebookDivisionRow)>);
    type IntoIter = StructuredSheetIterator<'a, NotebookDivisionSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, NotebookDivisionSheet> {
        StructuredSheetIterator {
            sheet: self,
            iterator: (&self.sheet).into_iter(),
        }
    }
}
#[derive(Clone, Debug, PartialEq)]
pub struct NotebookDivisionRow {
    ///""
    pub Name: String,
    ///""
    pub QuestUnlock: u32,
    ///""
    pub NotebookDivisionCategory: u8,
    ///""
    pub CraftOpeningLevel: u8,
    ///""
    pub GatheringOpeningLevel: u8,
    ///""
    pub Unknown1: u8,
    ///""
    pub Unknown0: u8,
    ///""
    pub SecretRecipeBookGroups: [u8; 2],
    ///"Index is CraftType"
    pub AllowedCraftTypes: [bool; 8],
    ///""
    pub RequiresSecretRecipeBookGroupUnlock: bool,
}
