...
impl StructuredSheet for ContentExActionSheet {
    type Row = ContentExActionRow;
...
impl<'a> IntoIterator for &'a ContentExActionSheet {
    type Item = (u32, Vec<(u16, ContentExActionRow)>);
...
impl ContentExActionRow {
    pub fn Name<'a>(&'a self) -> [&'a Field; 2] {
        [&self.columns[0], &self.columns[1]]
    }
    pub fn Charges<'a>(&'a self) -> [&'a Field; 2] {
