...
impl<'a> StructuredSheet<'a> for TopicSelectSheet {
    type Row = TopicSelectRow<'a>;
...
impl<'a> IntoIterator for &'a TopicSelectSheet {
    type Item = (u32, Vec<(u16, TopicSelectRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct TopicSelectRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> TopicSelectRow<'a> {
    pub fn Name(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn Shop(&'a self) -> [&'a Field; 10] {
        [
