...
impl<'a> StructuredSheet<'a> for MainCommandSheet {
    type Row = MainCommandRow<'a>;
...
impl<'a> IntoIterator for &'a MainCommandSheet {
    type Item = (u32, Vec<(u16, MainCommandRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MainCommandRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> MainCommandRow<'a> {
    ...
    pub fn MainCommandCategory(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[4]]
    }
    pub fn Unknown0(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[5]]
    }
    pub fn SortID(&'a self) -> &'a Field {
