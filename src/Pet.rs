//! This file is auto-generated, do not edit it manually! This is generated based on the schema from https://github.com/xivdev/EXDSchema.
#![allow(warnings)]
use crate::{StructuredSheet, StructuredSheetIterator};
use physis::{
    Error, resource::{Resource, ResourceResolver},
    exd::EXD, exh::{EXH, ExcelColumnDefinition},
    excel::{Sheet, Field, Row},
    Language,
};
#[derive(Debug, Clone)]
pub struct PetSheet {
    sheet: Sheet,
}
impl PetSheet {
    /// Read the sheet from a `ResourceResolver`.
    pub fn read_from(
        resolver: &mut ResourceResolver,
        language: Language,
    ) -> Result<Self, Error> {
        let exh = resolver.read_excel_sheet_header("Pet")?;
        let sheet = resolver.read_excel_sheet(&exh, "Pet", language)?;
        Ok(Self { sheet })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn row(&self, row_id: u32) -> Option<PetRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<PetRow> {
        let row = &self.sheet.subrow(row_id, subrow_id)?;
        self.read_row(row)
    }
    /// Returns the number of rows in this sheet.
    pub fn row_count(&self) -> u32 {
        self.sheet.exh.header.row_count
    }
}
impl StructuredSheet for PetSheet {
    type Row = PetRow;
    fn read_row(&self, row: &Row) -> Option<Self::Row> {
        let column_defs = &self.sheet.exh.column_definitions;
        let mut zipped: Vec<_> = row
            .columns
            .clone()
            .into_iter()
            .zip(column_defs)
            .collect();
        zipped.sort_by(|(_, a_col), (_, b_col)| a_col.offset.cmp(&b_col.offset));
        let (columns, _): (Vec<Field>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(Self::Row { columns })
    }
}
impl<'a> IntoIterator for &'a PetSheet {
    type Item = (u32, Vec<(u16, PetRow)>);
    type IntoIter = StructuredSheetIterator<'a, PetSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, PetSheet> {
        StructuredSheetIterator {
            sheet: self,
            iterator: (&self.sheet).into_iter(),
        }
    }
}
#[derive(Debug, Clone)]
pub struct PetRow {
    columns: Vec<Field>,
}
impl PetRow {
    pub fn Name<'a>(&'a self) -> &'a Field {
        &self.columns[0]
    }
    pub fn Abilities<'a>(&'a self) -> [&'a Field; 4] {
        [&self.columns[1], &self.columns[2], &self.columns[3], &self.columns[4]]
    }
    pub fn AutoAction<'a>(&'a self) -> &'a Field {
        &self.columns[5]
    }
    pub fn SmallScalePercentage<'a>(&'a self) -> &'a Field {
        &self.columns[6]
    }
    pub fn MediumScalePercentage<'a>(&'a self) -> &'a Field {
        &self.columns[7]
    }
    pub fn LargeScalePercentage<'a>(&'a self) -> &'a Field {
        &self.columns[8]
    }
    pub fn Unknown8<'a>(&'a self) -> &'a Field {
        &self.columns[9]
    }
    pub fn Unknown9<'a>(&'a self) -> &'a Field {
        &self.columns[10]
    }
    pub fn Unknown10<'a>(&'a self) -> &'a Field {
        &self.columns[11]
    }
    pub fn Unknown11<'a>(&'a self) -> &'a Field {
        &self.columns[12]
    }
    pub fn Unknown12<'a>(&'a self) -> &'a Field {
        &self.columns[13]
    }
    pub fn Unknown13<'a>(&'a self) -> &'a Field {
        &self.columns[14]
    }
    pub fn Unknown14<'a>(&'a self) -> &'a Field {
        &self.columns[15]
    }
    pub fn Unknown15<'a>(&'a self) -> &'a Field {
        &self.columns[16]
    }
    pub fn NonCombatSummon<'a>(&'a self) -> &'a Field {
        &self.columns[17]
    }
    pub fn Unknown17<'a>(&'a self) -> &'a Field {
        &self.columns[18]
    }
}
