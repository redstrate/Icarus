...
impl BuddyEquipSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "BuddyEquip")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "BuddyEquip", &exh, language, i)?);
        }
...
impl BuddyEquipRow {
    ...
    pub fn ModelTop<'a>(&'a self) -> &'a ColumnData {
        &self.columns[9]
    }
    ...
    pub fn IconHead<'a>(&'a self) -> &'a ColumnData {
        &self.columns[12]
    }
    pub fn IconBody<'a>(&'a self) -> &'a ColumnData {
        &self.columns[13]
    }
    pub fn IconLegs<'a>(&'a self) -> &'a ColumnData {
        &self.columns[14]
    }
