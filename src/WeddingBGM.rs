...
impl<'a> StructuredSheet<'a> for WeddingBGMSheet {
    type Row = WeddingBGMRow<'a>;
...
impl<'a> IntoIterator for &'a WeddingBGMSheet {
    type Item = (u32, Vec<(u16, WeddingBGMRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct WeddingBGMRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> WeddingBGMRow<'a> {
    pub fn SongName(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn Song(&'a self) -> &'a Field {
