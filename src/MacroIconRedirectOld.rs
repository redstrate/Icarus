...
impl<'a> StructuredSheet<'a> for MacroIconRedirectOldSheet {
    type Row = MacroIconRedirectOldRow<'a>;
...
impl<'a> IntoIterator for &'a MacroIconRedirectOldSheet {
    type Item = (u32, Vec<(u16, MacroIconRedirectOldRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MacroIconRedirectOldRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> MacroIconRedirectOldRow<'a> {
    pub fn IconOld(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn IconNew(&'a self) -> &'a Field {
