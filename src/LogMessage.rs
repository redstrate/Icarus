...
impl<'a> StructuredSheet<'a> for LogMessageSheet {
    type Row = LogMessageRow<'a>;
...
impl<'a> IntoIterator for &'a LogMessageSheet {
    type Item = (u32, Vec<(u16, LogMessageRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct LogMessageRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> LogMessageRow<'a> {
    pub fn Text(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn LogKind(&'a self) -> &'a Field {
