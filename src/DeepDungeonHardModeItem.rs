...
impl<'a> StructuredSheet<'a> for DeepDungeonHardModeItemSheet {
    type Row = DeepDungeonHardModeItemRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a DeepDungeonHardModeItemSheet {
    type Item = (u32, Vec<(u16, DeepDungeonHardModeItemRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct DeepDungeonHardModeItemRow<'a> {
    row: &'a Row,
}
impl<'a> DeepDungeonHardModeItemRow<'a> {
