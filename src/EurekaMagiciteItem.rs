...
impl EurekaMagiciteItemSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "EurekaMagiciteItem")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "EurekaMagiciteItem", &exh, language, i)?,
                );
        }
...
impl EurekaMagiciteItemRow {
    ...
    pub fn EurekaMagiciteItemType<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
