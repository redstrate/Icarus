...
impl<'a> StructuredSheet<'a> for EurekaMagiciteItemSheet {
    type Row = EurekaMagiciteItemRow<'a>;
...
impl<'a> IntoIterator for &'a EurekaMagiciteItemSheet {
    type Item = (u32, Vec<(u16, EurekaMagiciteItemRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct EurekaMagiciteItemRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> EurekaMagiciteItemRow<'a> {
    pub fn Item(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn EurekaMagiciteItemType(&'a self) -> &'a Field {
