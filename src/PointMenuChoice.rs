...
impl<'a> StructuredSheet<'a> for PointMenuChoiceSheet {
    type Row = PointMenuChoiceRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a PointMenuChoiceSheet {
    type Item = (u32, Vec<(u16, PointMenuChoiceRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct PointMenuChoiceRow<'a> {
    row: &'a Row,
}
impl<'a> PointMenuChoiceRow<'a> {
