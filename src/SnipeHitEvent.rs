...
impl<'a> StructuredSheet<'a> for SnipeHitEventSheet {
    type Row = SnipeHitEventRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a SnipeHitEventSheet {
    type Item = (u32, Vec<(u16, SnipeHitEventRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct SnipeHitEventRow<'a> {
    row: &'a Row,
}
impl<'a> SnipeHitEventRow<'a> {
