...
impl GilShopItemSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "GilShopItem")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "GilShopItem", &exh, language, i)?);
        }
...
impl GilShopItemRow {
    ...
    pub fn QuestRequired<'a>(&'a self) -> [&'a ColumnData; 2] {
        [&self.columns[1], &self.columns[2]]
    }
    pub fn AchievementRequired<'a>(&'a self) -> &'a ColumnData {
        &self.columns[3]
    }
    pub fn StateRequired<'a>(&'a self) -> &'a ColumnData {
        &self.columns[4]
    }
    ...
    pub fn IsHQ<'a>(&'a self) -> &'a ColumnData {
        &self.columns[9]
    }
}
