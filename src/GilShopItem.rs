...
impl<'a> StructuredSheet<'a> for GilShopItemSheet {
    type Row = GilShopItemRow<'a>;
...
impl<'a> IntoIterator for &'a GilShopItemSheet {
    type Item = (u32, Vec<(u16, GilShopItemRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct GilShopItemRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> GilShopItemRow<'a> {
    pub fn Item(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn QuestRequired(&'a self) -> [&'a Field; 2] {
        [
            &self.row.columns[self.index_mapping[1]],
            &self.row.columns[self.index_mapping[2]],
        ]
    }
    pub fn AchievementRequired(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn StateRequired(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[4]]
    }
    pub fn Patch(&'a self) -> &'a Field {
