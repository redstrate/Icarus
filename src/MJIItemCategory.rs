...
impl<'a> StructuredSheet<'a> for MJIItemCategorySheet {
    type Row = MJIItemCategoryRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a MJIItemCategorySheet {
    type Item = (u32, Vec<(u16, MJIItemCategoryRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MJIItemCategoryRow<'a> {
    row: &'a Row,
}
impl<'a> MJIItemCategoryRow<'a> {
