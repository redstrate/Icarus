...
impl<'a> StructuredSheet<'a> for FateShopSheet {
    type Row = FateShopRow<'a>;
...
impl<'a> IntoIterator for &'a FateShopSheet {
    type Item = (u32, Vec<(u16, FateShopRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct FateShopRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> FateShopRow<'a> {
    pub fn SpecialShop(&'a self) -> [&'a Field; 3] {
        [
    ...
    pub fn DefaultTalk(&'a self) -> [&'a Field; 9] {
        [
