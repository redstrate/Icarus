...
impl EventItemSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "EventItem")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "EventItem", &exh, language, i)?);
        }
...
impl EventItemRow {
    ...
    pub fn CastTimeline<'a>(&'a self) -> &'a ColumnData {
        &self.columns[16]
    }
    pub fn Timeline<'a>(&'a self) -> &'a ColumnData {
        &self.columns[17]
    }
}
