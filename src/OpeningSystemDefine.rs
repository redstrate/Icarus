...
impl<'a> StructuredSheet<'a> for OpeningSystemDefineSheet {
    type Row = OpeningSystemDefineRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a OpeningSystemDefineSheet {
    type Item = (u32, Vec<(u16, OpeningSystemDefineRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct OpeningSystemDefineRow<'a> {
    row: &'a Row,
}
impl<'a> OpeningSystemDefineRow<'a> {
