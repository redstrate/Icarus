...
impl GcArmyCaptureTacticsSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "GcArmyCaptureTactics")?;
...
impl GcArmyCaptureTacticsRow {
    pub fn Tactic<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    ...
    pub fn DamageDealt<'a>(&'a self) -> &'a ColumnData {
        &self.columns[4]
    }
    pub fn DamageReceived<'a>(&'a self) -> &'a ColumnData {
        &self.columns[5]
    }
}
