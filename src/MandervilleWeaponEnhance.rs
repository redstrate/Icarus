...
impl MandervilleWeaponEnhanceRow {
    pub fn Materia<'a>(&'a self) -> [&'a Field; 4] {
        [&self.columns[0], &self.columns[1], &self.columns[2], &self.columns[3]]
    }
    pub fn UpgradeItem<'a>(&'a self) -> &'a Field {
        &self.columns[4]
    }
    pub fn Image<'a>(&'a self) -> &'a Field {
        &self.columns[5]
    }
    pub fn UpgradeCost<'a>(&'a self) -> &'a Field {
        &self.columns[6]
    }
    pub fn Unknown7<'a>(&'a self) -> &'a Field {
        &self.columns[7]
    }
    pub fn MateriaBigValueIndex<'a>(&'a self) -> [&'a Field; 4] {
        [&self.columns[8], &self.columns[9], &self.columns[10], &self.columns[11]]
    }
    pub fn MateriaSmallValueIndex<'a>(&'a self) -> [&'a Field; 4] {
