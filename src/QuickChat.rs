...
impl<'a> StructuredSheet<'a> for QuickChatSheet {
    type Row = QuickChatRow<'a>;
...
impl<'a> IntoIterator for &'a QuickChatSheet {
    type Item = (u32, Vec<(u16, QuickChatRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct QuickChatRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> QuickChatRow<'a> {
    pub fn NameAction(&'a self) -> &'a Field {
    ...
    pub fn QuickChatTransient(&'a self) -> &'a Field {
