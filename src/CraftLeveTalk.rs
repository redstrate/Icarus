...
impl CraftLeveTalkSheet {
    ...
    pub fn read_from(
        ...
        let exh = resolver.read_excel_sheet_header("CraftLeveTalk")?;
        let sheet = resolver.read_excel_sheet(&exh, "CraftLeveTalk", language)?;
        Ok(Self { sheet })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn row(&self, row_id: u32) -> Option<CraftLeveTalkRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<CraftLeveTalkRow> {
...
impl<'a> StructuredSheet<'a> for CraftLeveTalkSheet {
    type Row = CraftLeveTalkRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a CraftLeveTalkSheet {
    type Item = (u32, Vec<(u16, CraftLeveTalkRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, CraftLeveTalkSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, CraftLeveTalkSheet> {
...
#[derive(Debug, Clone)]
pub struct CraftLeveTalkRow<'a> {
    row: &'a Row,
}
impl<'a> CraftLeveTalkRow<'a> {
