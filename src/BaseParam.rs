//! This file is auto-generated, do not edit it manually! This is generated based on the schema from https://github.com/xivdev/EXDSchema.
#![allow(warnings)]
use crate::{StructuredSheet, StructuredSheetIterator};
use physis::{
    Error, resource::{Resource, ResourceResolver},
    exd::EXD, exh::{EXH, ExcelColumnDefinition},
    excel::{Sheet, Field, Row},
    Language,
};
#[derive(Debug, Clone)]
pub struct BaseParamSheet {
    sheet: Sheet,
}
impl BaseParamSheet {
    /// Read the sheet from a `ResourceResolver`.
    pub fn read_from(
        resolver: &mut ResourceResolver,
        language: Language,
    ) -> Result<Self, Error> {
        let exh = resolver.read_excel_sheet_header("BaseParam")?;
        let sheet = resolver.read_excel_sheet(&exh, "BaseParam", language)?;
        Ok(Self { sheet })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn row(&self, row_id: u32) -> Option<BaseParamRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<BaseParamRow> {
        let row = &self.sheet.subrow(row_id, subrow_id)?;
        self.read_row(row)
    }
    /// Returns the number of rows in this sheet.
    pub fn row_count(&self) -> u32 {
        self.sheet.exh.header.row_count
    }
}
impl StructuredSheet for BaseParamSheet {
    type Row = BaseParamRow;
    fn read_row(&self, row: &Row) -> Option<Self::Row> {
        Some(Self::Row {
            Name: row
                .columns[1]
                .into_string()
                .cloned()
                .expect("Expected column 1 to be a string!"),
            Description: row
                .columns[2]
                .into_string()
                .cloned()
                .expect("Expected column 2 to be a string!"),
            OneHandWeaponPercent: row
                .columns[4]
                .into_u16()
                .copied()
                .expect("Expected column 4 to be a uint16!"),
            OffHandPercent: row
                .columns[5]
                .into_u16()
                .copied()
                .expect("Expected column 5 to be a uint16!"),
            HeadPercent: row
                .columns[6]
                .into_u16()
                .copied()
                .expect("Expected column 6 to be a uint16!"),
            ChestPercent: row
                .columns[7]
                .into_u16()
                .copied()
                .expect("Expected column 7 to be a uint16!"),
            HandsPercent: row
                .columns[8]
                .into_u16()
                .copied()
                .expect("Expected column 8 to be a uint16!"),
            WaistPercent: row
                .columns[9]
                .into_u16()
                .copied()
                .expect("Expected column 9 to be a uint16!"),
            LegsPercent: row
                .columns[10]
                .into_u16()
                .copied()
                .expect("Expected column 10 to be a uint16!"),
            FeetPercent: row
                .columns[11]
                .into_u16()
                .copied()
                .expect("Expected column 11 to be a uint16!"),
            EarringPercent: row
                .columns[12]
                .into_u16()
                .copied()
                .expect("Expected column 12 to be a uint16!"),
            NecklacePercent: row
                .columns[13]
                .into_u16()
                .copied()
                .expect("Expected column 13 to be a uint16!"),
            BraceletPercent: row
                .columns[14]
                .into_u16()
                .copied()
                .expect("Expected column 14 to be a uint16!"),
            RingPercent: row
                .columns[15]
                .into_u16()
                .copied()
                .expect("Expected column 15 to be a uint16!"),
            TwoHandWeaponPercent: row
                .columns[16]
                .into_u16()
                .copied()
                .expect("Expected column 16 to be a uint16!"),
            UnderArmorPercent: row
                .columns[17]
                .into_u16()
                .copied()
                .expect("Expected column 17 to be a uint16!"),
            ChestHeadPercent: row
                .columns[18]
                .into_u16()
                .copied()
                .expect("Expected column 18 to be a uint16!"),
            ChestHeadLegsFeetPercent: row
                .columns[19]
                .into_u16()
                .copied()
                .expect("Expected column 19 to be a uint16!"),
            Unknown0: row
                .columns[20]
                .into_u16()
                .copied()
                .expect("Expected column 20 to be a uint16!"),
            LegsFeetPercent: row
                .columns[21]
                .into_u16()
                .copied()
                .expect("Expected column 21 to be a uint16!"),
            HeadChestHandsLegsFeetPercent: row
                .columns[22]
                .into_u16()
                .copied()
                .expect("Expected column 22 to be a uint16!"),
            ChestLegsGlovesPercent: row
                .columns[23]
                .into_u16()
                .copied()
                .expect("Expected column 23 to be a uint16!"),
            ChestLegsFeetPercent: row
                .columns[24]
                .into_u16()
                .copied()
                .expect("Expected column 24 to be a uint16!"),
            Unknown1: row
                .columns[25]
                .into_u16()
                .copied()
                .expect("Expected column 25 to be a uint16!"),
            Unknown3: row
                .columns[26]
                .into_u16()
                .copied()
                .expect("Expected column 26 to be a uint16!"),
            OrderPriority: row
                .columns[3]
                .into_u8()
                .copied()
                .expect("Expected column 3 to be a uint8!"),
            MeldParam: [
                row
                    .columns[27]
                    .into_u8()
                    .copied()
                    .expect("Expected column 27 to be a uint8!"),
                row
                    .columns[28]
                    .into_u8()
                    .copied()
                    .expect("Expected column 28 to be a uint8!"),
                row
                    .columns[29]
                    .into_u8()
                    .copied()
                    .expect("Expected column 29 to be a uint8!"),
                row
                    .columns[30]
                    .into_u8()
                    .copied()
                    .expect("Expected column 30 to be a uint8!"),
                row
                    .columns[31]
                    .into_u8()
                    .copied()
                    .expect("Expected column 31 to be a uint8!"),
                row
                    .columns[32]
                    .into_u8()
                    .copied()
                    .expect("Expected column 32 to be a uint8!"),
                row
                    .columns[33]
                    .into_u8()
                    .copied()
                    .expect("Expected column 33 to be a uint8!"),
                row
                    .columns[34]
                    .into_u8()
                    .copied()
                    .expect("Expected column 34 to be a uint8!"),
                row
                    .columns[35]
                    .into_u8()
                    .copied()
                    .expect("Expected column 35 to be a uint8!"),
                row
                    .columns[36]
                    .into_u8()
                    .copied()
                    .expect("Expected column 36 to be a uint8!"),
                row
                    .columns[37]
                    .into_u8()
                    .copied()
                    .expect("Expected column 37 to be a uint8!"),
                row
                    .columns[38]
                    .into_u8()
                    .copied()
                    .expect("Expected column 38 to be a uint8!"),
                row
                    .columns[39]
                    .into_u8()
                    .copied()
                    .expect("Expected column 39 to be a uint8!"),
            ],
            PacketIndex: row
                .columns[0]
                .into_i8()
                .copied()
                .expect("Expected column 0 to be a int8!"),
            Unknown2: row
                .columns[40]
                .into_bool()
                .copied()
                .expect("Expected column 40 to be a bool!"),
        })
    }
}
impl<'a> IntoIterator for &'a BaseParamSheet {
    type Item = (u32, Vec<(u16, BaseParamRow)>);
    type IntoIter = StructuredSheetIterator<'a, BaseParamSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, BaseParamSheet> {
        StructuredSheetIterator {
            sheet: self,
            iterator: (&self.sheet).into_iter(),
        }
    }
}
#[derive(Clone, Debug, PartialEq)]
pub struct BaseParamRow {
    ///""
    pub Name: String,
    ///""
    pub Description: String,
    ///""
    pub OneHandWeaponPercent: u16,
    ///""
    pub OffHandPercent: u16,
    ///""
    pub HeadPercent: u16,
    ///""
    pub ChestPercent: u16,
    ///""
    pub HandsPercent: u16,
    ///""
    pub WaistPercent: u16,
    ///""
    pub LegsPercent: u16,
    ///""
    pub FeetPercent: u16,
    ///""
    pub EarringPercent: u16,
    ///""
    pub NecklacePercent: u16,
    ///""
    pub BraceletPercent: u16,
    ///""
    pub RingPercent: u16,
    ///""
    pub TwoHandWeaponPercent: u16,
    ///""
    pub UnderArmorPercent: u16,
    ///""
    pub ChestHeadPercent: u16,
    ///""
    pub ChestHeadLegsFeetPercent: u16,
    ///""
    pub Unknown0: u16,
    ///""
    pub LegsFeetPercent: u16,
    ///""
    pub HeadChestHandsLegsFeetPercent: u16,
    ///""
    pub ChestLegsGlovesPercent: u16,
    ///""
    pub ChestLegsFeetPercent: u16,
    ///""
    pub Unknown1: u16,
    ///""
    pub Unknown3: u16,
    ///""
    pub OrderPriority: u8,
    ///""
    pub MeldParam: [u8; 13],
    ///""
    pub PacketIndex: i8,
    ///""
    pub Unknown2: bool,
}
