...
impl<'a> StructuredSheet<'a> for TutorialSheet {
    type Row = TutorialRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a TutorialSheet {
    type Item = (u32, Vec<(u16, TutorialRow<'a>)>);
