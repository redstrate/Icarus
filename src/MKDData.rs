...
impl MKDDataSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "MKDData")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "MKDData", &exh, language, i)?);
        }
...
impl MKDDataRow {
    ...
    pub fn ZoneName<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn CurrencyItem<'a>(&'a self) -> [&'a ColumnData; 2] {
        [&self.columns[2], &self.columns[3]]
    }
    pub fn CipherItem<'a>(&'a self) -> &'a ColumnData {
        &self.columns[4]
    }
    pub fn CurrencyName<'a>(&'a self) -> [&'a ColumnData; 2] {
        [&self.columns[5], &self.columns[6]]
    }
    pub fn CipherName<'a>(&'a self) -> &'a ColumnData {
        &self.columns[7]
    }
