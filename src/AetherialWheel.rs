...
impl<'a> StructuredSheet<'a> for AetherialWheelSheet {
    type Row = AetherialWheelRow<'a>;
...
impl<'a> IntoIterator for &'a AetherialWheelSheet {
    type Item = (u32, Vec<(u16, AetherialWheelRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct AetherialWheelRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> AetherialWheelRow<'a> {
    pub fn ItemUnprimed(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn ItemPrimed(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn Grade(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn HoursRequired(&'a self) -> &'a Field {
