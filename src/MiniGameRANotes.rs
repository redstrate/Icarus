...
impl<'a> StructuredSheet<'a> for MiniGameRANotesSheet {
    type Row = MiniGameRANotesRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a MiniGameRANotesSheet {
    type Item = (u32, Vec<(u16, MiniGameRANotesRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MiniGameRANotesRow<'a> {
    row: &'a Row,
}
impl<'a> MiniGameRANotesRow<'a> {
