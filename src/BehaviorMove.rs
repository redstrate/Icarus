...
impl<'a> StructuredSheet<'a> for BehaviorMoveSheet {
    type Row = BehaviorMoveRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a BehaviorMoveSheet {
    type Item = (u32, Vec<(u16, BehaviorMoveRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct BehaviorMoveRow<'a> {
    row: &'a Row,
}
impl<'a> BehaviorMoveRow<'a> {
