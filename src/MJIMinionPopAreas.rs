...
impl MJIMinionPopAreasSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "MJIMinionPopAreas")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "MJIMinionPopAreas", &exh, language, i)?,
                );
        }
...
impl MJIMinionPopAreasRow {
    ...
    pub fn RequiredFunction<'a>(&'a self) -> &'a ColumnData {
        &self.columns[3]
    }
