...
impl<'a> StructuredSheet<'a> for BGMSituationSheet {
    type Row = BGMSituationRow<'a>;
...
impl<'a> IntoIterator for &'a BGMSituationSheet {
    type Item = (u32, Vec<(u16, BGMSituationRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct BGMSituationRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> BGMSituationRow<'a> {
    pub fn DaytimeID(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn NightID(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn BattleID(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn DaybreakID(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn TwilightID(&'a self) -> &'a Field {
