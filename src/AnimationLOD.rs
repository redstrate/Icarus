...
impl AnimationLODSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "AnimationLOD")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "AnimationLOD", &exh, language, i)?);
        }
...
impl AnimationLODRow {
    pub fn CameraDistance<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn SampleInterval<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn BoneLOD<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn AnimationEnable<'a>(&'a self) -> [&'a ColumnData; 8] {
        [
            &self.columns[3],
            &self.columns[4],
            &self.columns[5],
