...
impl DeepDungeonSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "DeepDungeon")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "DeepDungeon", &exh, language, i)?);
        }
...
impl DeepDungeonRow {
    ...
    pub fn ContentFinderConditionStart<'a>(&'a self) -> &'a ColumnData {
        &self.columns[34]
    }
