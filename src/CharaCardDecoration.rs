...
impl CharaCardDecorationSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "CharaCardDecoration")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "CharaCardDecoration", &exh, language, i)?,
                );
        }
...
impl CharaCardDecorationRow {
    ...
    pub fn Subtype<'a>(&'a self) -> &'a ColumnData {
        &self.columns[8]
    }
