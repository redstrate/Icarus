...
impl<'a> StructuredSheet<'a> for FCActivityCategorySheet {
    type Row = FCActivityCategoryRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a FCActivityCategorySheet {
    type Item = (u32, Vec<(u16, FCActivityCategoryRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct FCActivityCategoryRow<'a> {
    row: &'a Row,
}
impl<'a> FCActivityCategoryRow<'a> {
