...
impl RetainerTaskParameterSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "RetainerTaskParameter")?;
...
impl RetainerTaskParameterRow {
    pub fn ItemLevelDoW<'a>(&'a self) -> [&'a ColumnData; 4] {
        [&self.columns[0], &self.columns[1], &self.columns[2], &self.columns[3]]
    }
    pub fn PerceptionDoL<'a>(&'a self) -> [&'a ColumnData; 4] {
        [&self.columns[4], &self.columns[5], &self.columns[6], &self.columns[7]]
    }
    pub fn PerceptionFSH<'a>(&'a self) -> [&'a ColumnData; 4] {
