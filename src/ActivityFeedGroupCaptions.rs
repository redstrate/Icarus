...
impl ActivityFeedGroupCaptionsSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "ActivityFeedGroupCaptions")?;
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<ActivityFeedGroupCaptionsRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(ActivityFeedGroupCaptionsRow {
            columns,
        })
    }
...
impl ActivityFeedGroupCaptionsRow {
    pub fn JA<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn EN<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn DE<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn FR<'a>(&'a self) -> &'a ColumnData {
        &self.columns[3]
    }
}
