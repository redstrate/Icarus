...
impl BeastReputationRankSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "BeastReputationRank")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "BeastReputationRank", &exh, language, i)?,
                );
        }
...
impl BeastReputationRankRow {
    ...
    pub fn AlliedNames<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn Color<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn RequiredReputation<'a>(&'a self) -> &'a ColumnData {
        &self.columns[3]
    }
}
