...
impl<'a> StructuredSheet<'a> for SatisfactionSupplySheet {
    type Row = SatisfactionSupplyRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a SatisfactionSupplySheet {
    type Item = (u32, Vec<(u16, SatisfactionSupplyRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct SatisfactionSupplyRow<'a> {
    row: &'a Row,
}
impl<'a> SatisfactionSupplyRow<'a> {
    pub fn Item(&'a self) -> i32 {
        self.row.columns[2].into_i32().copied().unwrap()
    }
    pub fn CollectabilityLow(&'a self) -> u16 {
        self.row.columns[3].into_u16().copied().unwrap()
    }
    pub fn CollectabilityMid(&'a self) -> u16 {
        self.row.columns[4].into_u16().copied().unwrap()
    }
    pub fn CollectabilityHigh(&'a self) -> u16 {
        self.row.columns[5].into_u16().copied().unwrap()
    }
    pub fn Reward(&'a self) -> u16 {
        self.row.columns[6].into_u16().copied().unwrap()
    }
    pub fn FishingSpotId(&'a self) -> u16 {
        self.row.columns[8].into_u16().copied().unwrap()
    }
    pub fn SpearFishingSpotId(&'a self) -> u16 {
        self.row.columns[9].into_u16().copied().unwrap()
    }
    pub fn Slot(&'a self) -> u8 {
        self.row.columns[0].into_u8().copied().unwrap()
    }
    pub fn ProbabilityPercent(&'a self) -> u8 {
        self.row.columns[1].into_u8().copied().unwrap()
    }
    pub fn IsBonus(&'a self) -> bool {
