...
impl QuestAcceptAdditionConditionSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "QuestAcceptAdditionCondition")?;
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<QuestAcceptAdditionConditionRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(QuestAcceptAdditionConditionRow {
            columns,
        })
    }
...
impl QuestAcceptAdditionConditionRow {
    pub fn Requirement0<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn Requirement1<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
