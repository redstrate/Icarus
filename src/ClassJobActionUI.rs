...
impl ClassJobActionUISheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "ClassJobActionUI")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "ClassJobActionUI", &exh, language, i)?,
                );
        }
...
impl ClassJobActionUIRow {
    pub fn UpgradeAction<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn BaseAction<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    ...
    pub fn ComboTreeLayout<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    ...
    pub fn GroupedCell<'a>(&'a self) -> &'a ColumnData {
        &self.columns[6]
    }
}
