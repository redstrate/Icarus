...
impl<'a> StructuredSheet<'a> for ActivityFeedButtonsSheet {
    type Row = ActivityFeedButtonsRow<'a>;
...
impl<'a> IntoIterator for &'a ActivityFeedButtonsSheet {
    type Item = (u32, Vec<(u16, ActivityFeedButtonsRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ActivityFeedButtonsRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> ActivityFeedButtonsRow<'a> {
    pub fn BannerURL(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn Description(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn Language(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn PictureURL(&'a self) -> &'a Field {
