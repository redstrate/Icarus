...
impl<'a> StructuredSheet<'a> for EventActionSheet {
    type Row = EventActionRow<'a>;
...
impl<'a> IntoIterator for &'a EventActionSheet {
    type Item = (u32, Vec<(u16, EventActionRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct EventActionRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> EventActionRow<'a> {
    ...
    pub fn StartAnimation(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn LoopAnimation(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn EndAnimation(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[4]]
    }
    pub fn CastTime(&'a self) -> &'a Field {
