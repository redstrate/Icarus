...
impl<'a> StructuredSheet<'a> for WKSMechaEventObjectGroupSheet {
    type Row = WKSMechaEventObjectGroupRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a WKSMechaEventObjectGroupSheet {
    type Item = (u32, Vec<(u16, WKSMechaEventObjectGroupRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct WKSMechaEventObjectGroupRow<'a> {
    row: &'a Row,
}
impl<'a> WKSMechaEventObjectGroupRow<'a> {
