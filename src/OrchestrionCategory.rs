...
impl OrchestrionCategorySheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "OrchestrionCategory")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "OrchestrionCategory", &exh, language, i)?,
                );
        }
...
impl OrchestrionCategoryRow {
    ...
    pub fn HideOrder<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
