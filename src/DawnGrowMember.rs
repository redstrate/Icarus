...
impl DawnGrowMemberSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "DawnGrowMember")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "DawnGrowMember", &exh, language, i)?);
        }
...
impl DawnGrowMemberRow {
    pub fn SelectImage<'a>(&'a self) -> [&'a ColumnData; 4] {
        [&self.columns[0], &self.columns[1], &self.columns[2], &self.columns[3]]
    }
    pub fn PortraitImage<'a>(&'a self) -> [&'a ColumnData; 4] {
