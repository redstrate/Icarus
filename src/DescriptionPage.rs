...
impl DescriptionPageSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "DescriptionPage")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(read_excel_sheet(resource, "DescriptionPage", &exh, language, i)?);
        }
...
impl DescriptionPageRow {
    ...
    pub fn Image<'a>(&'a self) -> [&'a ColumnData; 11] {
        [
            &self.columns[2],
            &self.columns[3],
            &self.columns[4],
    ...
    pub fn Text<'a>(&'a self) -> [&'a ColumnData; 11] {
        [
            &self.columns[13],
            &self.columns[14],
            &self.columns[15],
