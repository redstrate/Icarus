...
impl<'a> StructuredSheet<'a> for WKSEmergencyProblemSheet {
    type Row = WKSEmergencyProblemRow<'a>;
...
impl<'a> IntoIterator for &'a WKSEmergencyProblemSheet {
    type Item = (u32, Vec<(u16, WKSEmergencyProblemRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct WKSEmergencyProblemRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> WKSEmergencyProblemRow<'a> {
    /// The first option to teleport
    pub fn EmergencyInfoTeleportText1(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    /// The second option to teleport
    pub fn EmergencyInfoTeleportText2(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn EventLayout(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn RequiredProgress(&'a self) -> &'a Field {
