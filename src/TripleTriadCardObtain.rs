...
impl<'a> StructuredSheet<'a> for TripleTriadCardObtainSheet {
    type Row = TripleTriadCardObtainRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a TripleTriadCardObtainSheet {
    type Item = (u32, Vec<(u16, TripleTriadCardObtainRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct TripleTriadCardObtainRow<'a> {
    row: &'a Row,
}
impl<'a> TripleTriadCardObtainRow<'a> {
