...
impl<'a> StructuredSheet<'a> for ContentGaugeSheet {
    type Row = ContentGaugeRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a ContentGaugeSheet {
    type Item = (u32, Vec<(u16, ContentGaugeRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ContentGaugeRow<'a> {
    row: &'a Row,
}
impl<'a> ContentGaugeRow<'a> {
    pub fn Name(&'a self) -> &'a str {
        self.row.columns[1].into_string().unwrap()
    }
    pub fn TextString(&'a self) -> &'a str {
