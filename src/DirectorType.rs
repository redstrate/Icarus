...
impl<'a> StructuredSheet<'a> for DirectorTypeSheet {
    type Row = DirectorTypeRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a DirectorTypeSheet {
    type Item = (u32, Vec<(u16, DirectorTypeRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct DirectorTypeRow<'a> {
    row: &'a Row,
}
impl<'a> DirectorTypeRow<'a> {
