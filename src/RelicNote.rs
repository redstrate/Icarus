...
impl RelicNoteSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "RelicNote")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "RelicNote", &exh, language, i)?);
        }
...
impl RelicNoteRow {
    ...
    pub fn MonsterNoteTargetCommon<'a>(&'a self) -> [&'a ColumnData; 10] {
        [
            &self.columns[1],
            &self.columns[2],
            &self.columns[3],
    ...
    pub fn MonsterNoteTargetNM<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[11], &self.columns[12], &self.columns[13]]
    }
    pub fn Unknown0<'a>(&'a self) -> &'a ColumnData {
        &self.columns[14]
    }
    pub fn Fate<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[15], &self.columns[16], &self.columns[17]]
    }
    pub fn PlaceNameFate<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[18], &self.columns[19], &self.columns[20]]
    }
    pub fn Leve<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[21], &self.columns[22], &self.columns[23]]
    }
    pub fn MonsterCount<'a>(&'a self) -> [&'a ColumnData; 10] {
        [
            &self.columns[24],
            &self.columns[25],
            &self.columns[26],
