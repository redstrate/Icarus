...
impl<'a> StructuredSheet<'a> for MapExclusiveSheet {
    type Row = MapExclusiveRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a MapExclusiveSheet {
    type Item = (u32, Vec<(u16, MapExclusiveRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MapExclusiveRow<'a> {
    row: &'a Row,
}
impl<'a> MapExclusiveRow<'a> {
