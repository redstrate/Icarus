...
impl<'a> StructuredSheet<'a> for GimmickRectSheet {
    type Row = GimmickRectRow<'a>;
...
impl<'a> IntoIterator for &'a GimmickRectSheet {
    type Item = (u32, Vec<(u16, GimmickRectRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct GimmickRectRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> GimmickRectRow<'a> {
    /// The EventRange to associate with.
    pub fn LayoutID(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn Params(&'a self) -> [&'a Field; 8] {
        [
    ...
    /// 8 = Jump
    pub fn TriggerIn(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[9]]
    }
    pub fn TriggerOut(&'a self) -> &'a Field {
