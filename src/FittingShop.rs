...
impl<'a> StructuredSheet<'a> for FittingShopSheet {
    type Row = FittingShopRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a FittingShopSheet {
    type Item = (u32, Vec<(u16, FittingShopRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct FittingShopRow<'a> {
    row: &'a Row,
}
impl<'a> FittingShopRow<'a> {
