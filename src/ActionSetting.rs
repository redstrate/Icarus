...
impl ActionSettingSheet {
    ...
    pub fn row(&self, row_id: u32) -> Option<ActionSettingRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<ActionSettingRow> {
...
impl<'a> StructuredSheet<'a> for ActionSettingSheet {
    type Row = ActionSettingRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a ActionSettingSheet {
    type Item = (u32, Vec<(u16, ActionSettingRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, ActionSettingSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, ActionSettingSheet> {
...
#[derive(Debug, Clone)]
pub struct ActionSettingRow<'a> {
    row: &'a Row,
}
impl<'a> ActionSettingRow<'a> {
