...
impl PublicContentSheet {
    ...
    pub fn row(&self, row_id: u32) -> Option<PublicContentRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<PublicContentRow> {
...
impl<'a> StructuredSheet<'a> for PublicContentSheet {
    type Row = PublicContentRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a PublicContentSheet {
    type Item = (u32, Vec<(u16, PublicContentRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, PublicContentSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, PublicContentSheet> {
...
#[derive(Debug, Clone)]
pub struct PublicContentRow<'a> {
    row: &'a Row,
}
impl<'a> PublicContentRow<'a> {
    ...
    pub fn AdditionalData(&'a self) -> u16 {
