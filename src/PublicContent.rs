...
impl PublicContentSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "PublicContent")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "PublicContent", &exh, language, i)?);
        }
...
impl PublicContentRow {
    ...
    pub fn LGBPopRange<'a>(&'a self) -> &'a ColumnData {
        &self.columns[6]
    }
