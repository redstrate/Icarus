...
impl MJIProgressSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "MJIProgress")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "MJIProgress", &exh, language, i)?);
        }
...
impl MJIProgressRow {
    pub fn Vision<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn Objective<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn PreviousObjective<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
