...
impl StructuredSheet for MJIProgressSheet {
    type Row = MJIProgressRow;
...
impl<'a> IntoIterator for &'a MJIProgressSheet {
    type Item = (u32, Vec<(u16, MJIProgressRow)>);
...
impl MJIProgressRow {
    pub fn Vision<'a>(&'a self) -> &'a Field {
        &self.columns[0]
    }
    pub fn Objective<'a>(&'a self) -> &'a Field {
        &self.columns[1]
    }
    pub fn PreviousObjective<'a>(&'a self) -> &'a Field {
        &self.columns[2]
    }
