...
impl MirageStoreSetItemLookupSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "MirageStoreSetItemLookup")?;
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<MirageStoreSetItemLookupRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(MirageStoreSetItemLookupRow {
            columns,
        })
    }
...
impl MirageStoreSetItemLookupRow {
    pub fn Item<'a>(&'a self) -> [&'a ColumnData; 5] {
        [
            &self.columns[0],
            &self.columns[1],
            &self.columns[2],
