...
impl<'a> StructuredSheet<'a> for UDS_PropertySheet {
    type Row = UDS_PropertyRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a UDS_PropertySheet {
    type Item = (u32, Vec<(u16, UDS_PropertyRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct UDS_PropertyRow<'a> {
    row: &'a Row,
}
impl<'a> UDS_PropertyRow<'a> {
