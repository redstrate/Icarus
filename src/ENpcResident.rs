...
impl ENpcResidentSheet {
    ...
    pub fn read_from(
        ...
        let exh = resolver.read_excel_sheet_header("ENpcResident")?;
        let sheet = resolver.read_excel_sheet(&exh, "ENpcResident", language)?;
        Ok(Self { sheet })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn row(&self, row_id: u32) -> Option<ENpcResidentRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<ENpcResidentRow> {
...
impl<'a> StructuredSheet<'a> for ENpcResidentSheet {
    type Row = ENpcResidentRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a ENpcResidentSheet {
    type Item = (u32, Vec<(u16, ENpcResidentRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, ENpcResidentSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, ENpcResidentSheet> {
...
#[derive(Debug, Clone)]
pub struct ENpcResidentRow<'a> {
    row: &'a Row,
}
impl<'a> ENpcResidentRow<'a> {
