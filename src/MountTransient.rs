...
impl<'a> StructuredSheet<'a> for MountTransientSheet {
    type Row = MountTransientRow<'a>;
...
impl<'a> IntoIterator for &'a MountTransientSheet {
    type Item = (u32, Vec<(u16, MountTransientRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MountTransientRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> MountTransientRow<'a> {
