...
impl<'a> StructuredSheet<'a> for ArchiveItemSheet {
    type Row = ArchiveItemRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a ArchiveItemSheet {
    type Item = (u32, Vec<(u16, ArchiveItemRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ArchiveItemRow<'a> {
    row: &'a Row,
}
impl<'a> ArchiveItemRow<'a> {
