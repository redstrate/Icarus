...
impl<'a> StructuredSheet<'a> for FCActivitySheet {
    type Row = FCActivityRow<'a>;
...
impl<'a> IntoIterator for &'a FCActivitySheet {
    type Item = (u32, Vec<(u16, FCActivityRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct FCActivityRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> FCActivityRow<'a> {
    pub fn Text(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn SelfKind(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn TargetKind(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn NumParam(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn FCActivityCategory(&'a self) -> &'a Field {
