...
impl<'a> StructuredSheet<'a> for DeepDungeonMap5XSheet {
    type Row = DeepDungeonMap5XRow<'a>;
...
impl<'a> IntoIterator for &'a DeepDungeonMap5XSheet {
    type Item = (u32, Vec<(u16, DeepDungeonMap5XRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct DeepDungeonMap5XRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> DeepDungeonMap5XRow<'a> {
    pub fn DeepDungeonRoom(&'a self) -> [&'a Field; 5] {
        [
