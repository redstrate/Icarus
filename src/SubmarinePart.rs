...
impl SubmarinePartSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "SubmarinePart")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "SubmarinePart", &exh, language, i)?);
        }
...
impl SubmarinePartRow {
    ...
    pub fn Surveillance<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn Retrieval<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    ...
    pub fn Favor<'a>(&'a self) -> &'a ColumnData {
        &self.columns[5]
    }
    ...
    pub fn Components<'a>(&'a self) -> &'a ColumnData {
        &self.columns[8]
    }
    pub fn RepairMaterials<'a>(&'a self) -> &'a ColumnData {
        &self.columns[9]
    }
}
