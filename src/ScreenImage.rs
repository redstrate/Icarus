...
impl ScreenImageSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "ScreenImage")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "ScreenImage", &exh, language, i)?);
        }
...
impl ScreenImageRow {
    ...
    pub fn Jingle<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn Type<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn Lang<'a>(&'a self) -> &'a ColumnData {
        &self.columns[3]
    }
}
