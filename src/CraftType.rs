...
impl<'a> StructuredSheet<'a> for CraftTypeSheet {
    type Row = CraftTypeRow<'a>;
...
impl<'a> IntoIterator for &'a CraftTypeSheet {
    type Item = (u32, Vec<(u16, CraftTypeRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct CraftTypeRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> CraftTypeRow<'a> {
    pub fn Name(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn MainPhysical(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn SubPhysical(&'a self) -> &'a Field {
