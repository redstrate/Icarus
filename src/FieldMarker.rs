...
impl FieldMarkerSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "FieldMarker")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "FieldMarker", &exh, language, i)?);
        }
...
impl FieldMarkerRow {
    ...
    pub fn UiIcon<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
