...
impl<'a> StructuredSheet<'a> for FieldMarkerSheet {
    type Row = FieldMarkerRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a FieldMarkerSheet {
    type Item = (u32, Vec<(u16, FieldMarkerRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct FieldMarkerRow<'a> {
    row: &'a Row,
}
impl<'a> FieldMarkerRow<'a> {
    pub fn Name(&'a self) -> &'a str {
        self.row.columns[3].into_string().unwrap()
    }
    pub fn VFX(&'a self) -> i32 {
        self.row.columns[0].into_i32().copied().unwrap()
    }
    pub fn UiIcon(&'a self) -> u16 {
        self.row.columns[1].into_u16().copied().unwrap()
    }
    pub fn MapIcon(&'a self) -> u16 {
