...
impl<'a> StructuredSheet<'a> for SnipePerformanceCameraSheet {
    type Row = SnipePerformanceCameraRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a SnipePerformanceCameraSheet {
    type Item = (u32, Vec<(u16, SnipePerformanceCameraRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct SnipePerformanceCameraRow<'a> {
    row: &'a Row,
}
impl<'a> SnipePerformanceCameraRow<'a> {
