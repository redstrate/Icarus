...
impl ResistanceWeaponAdjustSheet {
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<ResistanceWeaponAdjustRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(ResistanceWeaponAdjustRow {
            columns,
        })
    }
...
impl ResistanceWeaponAdjustRow {
    ...
    pub fn MaxTotalStats<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn MaxEachStat<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn BaseParam<'a>(&'a self) -> [&'a ColumnData; 4] {
