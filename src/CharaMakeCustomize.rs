...
impl CharaMakeCustomizeSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "CharaMakeCustomize")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "CharaMakeCustomize", &exh, language, i)?,
                );
        }
...
impl CharaMakeCustomizeRow {
    ...
    pub fn Hint<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn HintItem<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn UnlockLink<'a>(&'a self) -> &'a ColumnData {
        &self.columns[3]
    }
    pub fn FeatureID<'a>(&'a self) -> &'a ColumnData {
        &self.columns[4]
    }
    pub fn Unknown0<'a>(&'a self) -> &'a ColumnData {
        &self.columns[5]
    }
    pub fn IsPurchasable<'a>(&'a self) -> &'a ColumnData {
        &self.columns[6]
    }
}
