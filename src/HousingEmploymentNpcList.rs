...
impl<'a> StructuredSheet<'a> for HousingEmploymentNpcListSheet {
    type Row = HousingEmploymentNpcListRow<'a>;
...
impl<'a> IntoIterator for &'a HousingEmploymentNpcListSheet {
    type Item = (u32, Vec<(u16, HousingEmploymentNpcListRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct HousingEmploymentNpcListRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> HousingEmploymentNpcListRow<'a> {
    pub fn MaleENpcBase(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn FemaleENpcBase(&'a self) -> &'a Field {
