...
impl<'a> StructuredSheet<'a> for CustomTalkDefineClientSheet {
    type Row = CustomTalkDefineClientRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a CustomTalkDefineClientSheet {
    type Item = (u32, Vec<(u16, CustomTalkDefineClientRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct CustomTalkDefineClientRow<'a> {
    row: &'a Row,
}
impl<'a> CustomTalkDefineClientRow<'a> {
