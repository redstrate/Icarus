...
impl<'a> StructuredSheet<'a> for HWDDevLivelySheet {
    type Row = HWDDevLivelyRow<'a>;
...
impl<'a> IntoIterator for &'a HWDDevLivelySheet {
    type Item = (u32, Vec<(u16, HWDDevLivelyRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct HWDDevLivelyRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> HWDDevLivelyRow<'a> {
