...
impl<'a> StructuredSheet<'a> for GoldSaucerArcadeMachineSheet {
    type Row = GoldSaucerArcadeMachineRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a GoldSaucerArcadeMachineSheet {
    type Item = (u32, Vec<(u16, GoldSaucerArcadeMachineRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct GoldSaucerArcadeMachineRow<'a> {
    row: &'a Row,
}
impl<'a> GoldSaucerArcadeMachineRow<'a> {
    pub fn Unknown0(&'a self) -> u32 {
        self.row.columns[27].into_u32().copied().unwrap()
    }
    pub fn Poor(&'a self) -> u32 {
