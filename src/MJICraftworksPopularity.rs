...
impl<'a> MJICraftworksPopularityRow<'a> {
    pub fn Popularity(&'a self) -> [u8; 91] {
        [
            ...
            self.row.columns[64].into_u8().copied().unwrap(),
            ...
            self.row.columns[74].into_u8().copied().unwrap(),
            ...
            self.row.columns[82].into_u8().copied().unwrap(),
