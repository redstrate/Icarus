...
impl RetainerTaskNormalSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "RetainerTaskNormal")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "RetainerTaskNormal", &exh, language, i)?,
                );
        }
...
impl RetainerTaskNormalRow {
    ...
    pub fn GatheringLog<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn FishingLog<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn Quantity<'a>(&'a self) -> [&'a ColumnData; 5] {
        [
            &self.columns[3],
            &self.columns[4],
            &self.columns[5],
