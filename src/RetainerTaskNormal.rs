...
impl StructuredSheet for RetainerTaskNormalSheet {
    type Row = RetainerTaskNormalRow;
...
impl<'a> IntoIterator for &'a RetainerTaskNormalSheet {
    type Item = (u32, Vec<(u16, RetainerTaskNormalRow)>);
...
impl RetainerTaskNormalRow {
    ...
    pub fn GatheringLog<'a>(&'a self) -> &'a Field {
        &self.columns[1]
    }
    pub fn FishingLog<'a>(&'a self) -> &'a Field {
        &self.columns[2]
    }
    pub fn Quantity<'a>(&'a self) -> [&'a Field; 5] {
        [
            &self.columns[3],
            &self.columns[4],
            &self.columns[5],
