...
impl ModelCharaSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "ModelChara")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "ModelChara", &exh, language, i)?);
        }
...
impl ModelCharaRow {
    ...
    pub fn PapVariation<'a>(&'a self) -> &'a ColumnData {
        &self.columns[16]
    }
