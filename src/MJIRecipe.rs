...
impl MJIRecipeSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "MJIRecipe")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "MJIRecipe", &exh, language, i)?);
        }
...
impl MJIRecipeRow {
    ...
    pub fn KeyItem<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn ItemPouch<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn Yield<'a>(&'a self) -> &'a ColumnData {
        &self.columns[3]
    }
