...
impl StructuredSheet for TextCommandSheet {
    type Row = TextCommandRow;
...
impl<'a> IntoIterator for &'a TextCommandSheet {
    type Item = (u32, Vec<(u16, TextCommandRow)>);
...
impl TextCommandRow {
    ...
    pub fn Alias<'a>(&'a self) -> &'a Field {
        &self.columns[1]
    }
    pub fn ShortAlias<'a>(&'a self) -> &'a Field {
        &self.columns[2]
    }
    pub fn Command<'a>(&'a self) -> &'a Field {
        &self.columns[3]
    }
    pub fn ShortCommand<'a>(&'a self) -> &'a Field {
        &self.columns[4]
    }
