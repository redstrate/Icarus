...
impl<'a> StructuredSheet<'a> for CreditListSheet {
    type Row = CreditListRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a CreditListSheet {
    type Item = (u32, Vec<(u16, CreditListRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct CreditListRow<'a> {
    row: &'a Row,
}
impl<'a> CreditListRow<'a> {
    pub fn Icon(&'a self) -> u32 {
        self.row.columns[1].into_u32().copied().unwrap()
    }
    pub fn Font(&'a self) -> u32 {
        self.row.columns[2].into_u32().copied().unwrap()
    }
    pub fn Cast(&'a self) -> u32 {
        self.row.columns[5].into_u32().copied().unwrap()
    }
    pub fn Scale(&'a self) -> u16 {
