...
impl OrnamentCustomizeSheet {
    ...
    pub fn row(&self, row_id: u32) -> Option<OrnamentCustomizeRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<OrnamentCustomizeRow> {
...
impl<'a> StructuredSheet<'a> for OrnamentCustomizeSheet {
    type Row = OrnamentCustomizeRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a OrnamentCustomizeSheet {
    type Item = (u32, Vec<(u16, OrnamentCustomizeRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, OrnamentCustomizeSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, OrnamentCustomizeSheet> {
...
#[derive(Debug, Clone)]
pub struct OrnamentCustomizeRow<'a> {
    row: &'a Row,
}
impl<'a> OrnamentCustomizeRow<'a> {
