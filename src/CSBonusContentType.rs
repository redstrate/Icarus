...
impl CSBonusContentTypeSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "CSBonusContentType")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "CSBonusContentType", &exh, language, i)?,
                );
        }
...
impl CSBonusContentTypeRow {
    pub fn Dialogue<'a>(&'a self) -> [&'a ColumnData; 4] {
