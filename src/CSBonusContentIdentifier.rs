...
impl<'a> StructuredSheet<'a> for CSBonusContentIdentifierSheet {
    type Row = CSBonusContentIdentifierRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a CSBonusContentIdentifierSheet {
    type Item = (u32, Vec<(u16, CSBonusContentIdentifierRow<'a>)>);
