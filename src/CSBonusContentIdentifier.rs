...
impl CSBonusContentIdentifierSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "CSBonusContentIdentifier")?;
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<CSBonusContentIdentifierRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(CSBonusContentIdentifierRow {
            columns,
        })
    }
...
impl CSBonusContentIdentifierRow {
    ...
    pub fn UnlockQuest0<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn UnlockQuest1<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn UnlockQuest2<'a>(&'a self) -> &'a ColumnData {
        &self.columns[3]
    }
