...
impl ItemFoodSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "ItemFood")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "ItemFood", &exh, language, i)?);
        }
...
impl ItemFoodRow {
    pub fn Max<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[0], &self.columns[1], &self.columns[2]]
    }
    pub fn MaxHQ<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[3], &self.columns[4], &self.columns[5]]
    }
    pub fn EXPBonusPercent<'a>(&'a self) -> &'a ColumnData {
        &self.columns[6]
    }
    pub fn BaseParam<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[7], &self.columns[8], &self.columns[9]]
    }
    pub fn Value<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[10], &self.columns[11], &self.columns[12]]
    }
    pub fn ValueHQ<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[13], &self.columns[14], &self.columns[15]]
    }
    pub fn IsRelative<'a>(&'a self) -> [&'a ColumnData; 3] {
