...
impl ChocoboTaxiStandSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "ChocoboTaxiStand")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "ChocoboTaxiStand", &exh, language, i)?,
                );
        }
...
impl ChocoboTaxiStandRow {
    ...
    pub fn TargetLocations<'a>(&'a self) -> [&'a ColumnData; 8] {
        [
            &self.columns[1],
            &self.columns[2],
            &self.columns[3],
