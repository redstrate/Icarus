...
impl<'a> StructuredSheet<'a> for WKSPioneeringTrailSheet {
    type Row = WKSPioneeringTrailRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a WKSPioneeringTrailSheet {
    type Item = (u32, Vec<(u16, WKSPioneeringTrailRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct WKSPioneeringTrailRow<'a> {
    row: &'a Row,
}
impl<'a> WKSPioneeringTrailRow<'a> {
