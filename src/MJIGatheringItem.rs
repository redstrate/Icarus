...
impl<'a> StructuredSheet<'a> for MJIGatheringItemSheet {
    type Row = MJIGatheringItemRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a MJIGatheringItemSheet {
    type Item = (u32, Vec<(u16, MJIGatheringItemRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MJIGatheringItemRow<'a> {
    row: &'a Row,
}
impl<'a> MJIGatheringItemRow<'a> {
