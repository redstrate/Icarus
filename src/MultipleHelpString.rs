...
impl<'a> StructuredSheet<'a> for MultipleHelpStringSheet {
    type Row = MultipleHelpStringRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a MultipleHelpStringSheet {
    type Item = (u32, Vec<(u16, MultipleHelpStringRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MultipleHelpStringRow<'a> {
    row: &'a Row,
}
impl<'a> MultipleHelpStringRow<'a> {
