...
impl GCScripShopItemSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "GCScripShopItem")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(read_excel_sheet(resource, "GCScripShopItem", &exh, language, i)?);
        }
...
impl GCScripShopItemRow {
    pub fn CostGCSeals<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn Item<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn RequiredGrandCompanyRank<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
