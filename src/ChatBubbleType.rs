...
impl<'a> StructuredSheet<'a> for ChatBubbleTypeSheet {
    type Row = ChatBubbleTypeRow<'a>;
...
impl<'a> IntoIterator for &'a ChatBubbleTypeSheet {
    type Item = (u32, Vec<(u16, ChatBubbleTypeRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ChatBubbleTypeRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> ChatBubbleTypeRow<'a> {
