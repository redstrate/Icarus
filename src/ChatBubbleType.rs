...
impl ChatBubbleTypeSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "ChatBubbleType")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "ChatBubbleType", &exh, language, i)?);
        }
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<ChatBubbleTypeRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(ChatBubbleTypeRow { columns })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn get_row(&self, row_id: u32) -> Option<ChatBubbleTypeRow> {
    ...
    pub fn get_subrow(&self, row_id: u32, subrow_id: u16) -> Option<ChatBubbleTypeRow> {
        for page in &self.pages {
            let Some(row) = &page.get_row(row_id) else {
                continue;
            };
