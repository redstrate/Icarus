...
impl<'a> StructuredSheet<'a> for MinionRulesSheet {
    type Row = MinionRulesRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a MinionRulesSheet {
    type Item = (u32, Vec<(u16, MinionRulesRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MinionRulesRow<'a> {
    row: &'a Row,
}
impl<'a> MinionRulesRow<'a> {
