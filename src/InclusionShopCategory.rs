...
impl<'a> StructuredSheet<'a> for InclusionShopCategorySheet {
    type Row = InclusionShopCategoryRow<'a>;
...
impl<'a> IntoIterator for &'a InclusionShopCategorySheet {
    type Item = (u32, Vec<(u16, InclusionShopCategoryRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct InclusionShopCategoryRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> InclusionShopCategoryRow<'a> {
    pub fn Name(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn InclusionShopSeries(&'a self) -> &'a Field {
