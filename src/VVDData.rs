...
impl VVDDataSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "VVDData")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "VVDData", &exh, language, i)?);
        }
...
impl VVDDataRow {
    ...
    pub fn ContentExAction<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    ...
    pub fn Series<'a>(&'a self) -> &'a ColumnData {
        &self.columns[4]
    }
