...
impl MonsterNoteTargetSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "MonsterNoteTarget")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "MonsterNoteTarget", &exh, language, i)?,
                );
        }
...
impl MonsterNoteTargetRow {
    ...
    pub fn PlaceNameZone<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[2], &self.columns[3], &self.columns[4]]
    }
    pub fn PlaceNameLocation<'a>(&'a self) -> [&'a ColumnData; 3] {
