...
impl<'a> StructuredSheet<'a> for UDS_EventSheet {
    type Row = UDS_EventRow<'a>;
...
impl<'a> IntoIterator for &'a UDS_EventSheet {
    type Item = (u32, Vec<(u16, UDS_EventRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct UDS_EventRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> UDS_EventRow<'a> {
    ...
    pub fn Property(&'a self) -> [&'a Field; 32] {
        [
