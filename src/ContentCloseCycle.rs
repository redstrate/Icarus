...
impl ContentCloseCycleSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "ContentCloseCycle")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "ContentCloseCycle", &exh, language, i)?,
                );
        }
...
impl ContentCloseCycleRow {
    pub fn Unixtime<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn TimeSeconds<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
