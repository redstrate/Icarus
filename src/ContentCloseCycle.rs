...
impl StructuredSheet for ContentCloseCycleSheet {
    type Row = ContentCloseCycleRow;
...
impl<'a> IntoIterator for &'a ContentCloseCycleSheet {
    type Item = (u32, Vec<(u16, ContentCloseCycleRow)>);
...
impl ContentCloseCycleRow {
    pub fn Unixtime<'a>(&'a self) -> &'a Field {
        &self.columns[0]
    }
    pub fn TimeSeconds<'a>(&'a self) -> &'a Field {
        &self.columns[1]
    }
