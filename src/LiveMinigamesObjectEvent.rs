...
impl LiveMinigamesObjectEventSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "LiveMinigamesObjectEvent")?;
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<LiveMinigamesObjectEventRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(LiveMinigamesObjectEventRow {
            columns,
        })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn get_row(&self, row_id: u32) -> Option<LiveMinigamesObjectEventRow> {
