...
impl<'a> StructuredSheet<'a> for LiveMinigamesObjectEventSheet {
    type Row = LiveMinigamesObjectEventRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a LiveMinigamesObjectEventSheet {
    type Item = (u32, Vec<(u16, LiveMinigamesObjectEventRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct LiveMinigamesObjectEventRow<'a> {
    row: &'a Row,
}
impl<'a> LiveMinigamesObjectEventRow<'a> {
    ...
    pub fn Unknown6(&'a self) -> f32 {
        self.row.columns[49].into_f32().copied().unwrap()
    }
    ...
    pub fn Unknown39(&'a self) -> i32 {
        self.row.columns[1].into_i32().copied().unwrap()
    }
    pub fn Unknown40(&'a self) -> i32 {
