...
impl<'a> StructuredSheet<'a> for GroupPoseStampCategorySheet {
    type Row = GroupPoseStampCategoryRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a GroupPoseStampCategorySheet {
    type Item = (u32, Vec<(u16, GroupPoseStampCategoryRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct GroupPoseStampCategoryRow<'a> {
    row: &'a Row,
}
impl<'a> GroupPoseStampCategoryRow<'a> {
