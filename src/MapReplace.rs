...
impl<'a> StructuredSheet<'a> for MapReplaceSheet {
    type Row = MapReplaceRow<'a>;
...
impl<'a> IntoIterator for &'a MapReplaceSheet {
    type Item = (u32, Vec<(u16, MapReplaceRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MapReplaceRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> MapReplaceRow<'a> {
