...
impl EObjNameSheet {
    ...
    pub fn row(&self, row_id: u32) -> Option<EObjNameRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<EObjNameRow> {
...
impl<'a> StructuredSheet<'a> for EObjNameSheet {
    type Row = EObjNameRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a EObjNameSheet {
    type Item = (u32, Vec<(u16, EObjNameRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, EObjNameSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, EObjNameSheet> {
...
#[derive(Debug, Clone)]
pub struct EObjNameRow<'a> {
    row: &'a Row,
}
impl<'a> EObjNameRow<'a> {
