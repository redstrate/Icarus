...
impl<'a> StructuredSheet<'a> for ContentRouletteRoleBonusSheet {
    type Row = ContentRouletteRoleBonusRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a ContentRouletteRoleBonusSheet {
    type Item = (u32, Vec<(u16, ContentRouletteRoleBonusRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ContentRouletteRoleBonusRow<'a> {
    row: &'a Row,
}
impl<'a> ContentRouletteRoleBonusRow<'a> {
    pub fn ItemRewardType(&'a self) -> u32 {
