...
impl CycleTimeSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "CycleTime")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "CycleTime", &exh, language, i)?);
        }
...
impl CycleTimeRow {
    pub fn FirstCycle<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn Cycle<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
}
