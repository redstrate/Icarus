...
impl ShellFixedFromCommandSheet {
    ...
    pub fn read_from(
        ...
        let exh = resolver.read_excel_sheet_header("ShellFixedFromCommand")?;
        let sheet = resolver.read_excel_sheet(&exh, "ShellFixedFromCommand", language)?;
        Ok(Self { sheet })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn row(&self, row_id: u32) -> Option<ShellFixedFromCommandRow> {
    ...
    pub fn subrow(
        ...
        row_id: u32,
        subrow_id: u16,
    ) -> Option<ShellFixedFromCommandRow> {
...
impl<'a> StructuredSheet<'a> for ShellFixedFromCommandSheet {
    type Row = ShellFixedFromCommandRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a ShellFixedFromCommandSheet {
    type Item = (u32, Vec<(u16, ShellFixedFromCommandRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, ShellFixedFromCommandSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, ShellFixedFromCommandSheet> {
...
#[derive(Debug, Clone)]
pub struct ShellFixedFromCommandRow<'a> {
    row: &'a Row,
}
impl<'a> ShellFixedFromCommandRow<'a> {
    pub fn Unknown0(&'a self) -> u64 {
        self.row.columns[8].into_u64().copied().unwrap()
    }
    pub fn Unknown1(&'a self) -> u64 {
        self.row.columns[9].into_u64().copied().unwrap()
    }
    pub fn Unknown2(&'a self) -> u64 {
        self.row.columns[10].into_u64().copied().unwrap()
    }
    pub fn Unknown3(&'a self) -> u64 {
        self.row.columns[11].into_u64().copied().unwrap()
    }
    pub fn Unknown4(&'a self) -> u64 {
        self.row.columns[12].into_u64().copied().unwrap()
    }
