...
impl PlayerSearchSubLocationSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "PlayerSearchSubLocation")?;
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<PlayerSearchSubLocationRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(PlayerSearchSubLocationRow {
            columns,
        })
    }
...
impl PlayerSearchSubLocationRow {
    pub fn Name0<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn Name1<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
