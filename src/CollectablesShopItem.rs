...
impl<'a> CollectablesShopItemRow<'a> {
    ...
    pub fn LevelMin(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn LevelMax(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn CollectablesShopRefine(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[4]]
    }
    pub fn CollectablesShopRewardScrip(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[5]]
    }
    pub fn CollectablesShopItemGroup(&'a self) -> &'a Field {
