...
impl CollectablesShopSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "CollectablesShop")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "CollectablesShop", &exh, language, i)?,
                );
        }
...
impl CollectablesShopRow {
    ...
    pub fn ShopItems<'a>(&'a self) -> [&'a ColumnData; 11] {
        [
            &self.columns[2],
            &self.columns[3],
            &self.columns[4],
    ...
    pub fn RewardType<'a>(&'a self) -> &'a ColumnData {
        &self.columns[13]
    }
}
