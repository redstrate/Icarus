...
impl<'a> StructuredSheet<'a> for TofuObjectSheet {
    type Row = TofuObjectRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a TofuObjectSheet {
    type Item = (u32, Vec<(u16, TofuObjectRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct TofuObjectRow<'a> {
    row: &'a Row,
}
impl<'a> TofuObjectRow<'a> {
