...
impl<'a> StructuredSheet<'a> for PetActionSheet {
    type Row = PetActionRow<'a>;
...
impl<'a> IntoIterator for &'a PetActionSheet {
    type Item = (u32, Vec<(u16, PetActionRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct PetActionRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> PetActionRow<'a> {
    ...
    pub fn Pet(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[4]]
    }
    pub fn MasterOrder(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[5]]
    }
    pub fn DisableOrder(&'a self) -> &'a Field {
