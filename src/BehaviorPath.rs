...
impl<'a> StructuredSheet<'a> for BehaviorPathSheet {
    type Row = BehaviorPathRow<'a>;
...
impl<'a> IntoIterator for &'a BehaviorPathSheet {
    type Item = (u32, Vec<(u16, BehaviorPathRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct BehaviorPathRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> BehaviorPathRow<'a> {
    pub fn Speed(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn IsTurnTransition(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn IsFadeOut(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn IsFadeIn(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn IsWalking(&'a self) -> &'a Field {
