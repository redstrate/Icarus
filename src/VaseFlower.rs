...
impl<'a> StructuredSheet<'a> for VaseFlowerSheet {
    type Row = VaseFlowerRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a VaseFlowerSheet {
    type Item = (u32, Vec<(u16, VaseFlowerRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct VaseFlowerRow<'a> {
    row: &'a Row,
}
impl<'a> VaseFlowerRow<'a> {
