...
impl WKSMissionToDoSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "WKSMissionToDo")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "WKSMissionToDo", &exh, language, i)?);
        }
...
impl WKSMissionToDoRow {
    pub fn TemporaryAction<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    ...
    pub fn RequiredItemQuantity<'a>(&'a self) -> [&'a ColumnData; 3] {
    ...
    pub fn MissionType<'a>(&'a self) -> &'a ColumnData {
        &self.columns[19]
    }
}
