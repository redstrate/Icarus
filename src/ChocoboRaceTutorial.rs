...
impl ChocoboRaceTutorialSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "ChocoboRaceTutorial")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "ChocoboRaceTutorial", &exh, language, i)?,
                );
        }
...
impl ChocoboRaceTutorialRow {
    pub fn NpcYell<'a>(&'a self) -> [&'a ColumnData; 8] {
        [
            &self.columns[0],
            &self.columns[1],
            &self.columns[2],
