...
impl<'a> StructuredSheet<'a> for ActionComboRouteSheet {
    type Row = ActionComboRouteRow<'a>;
...
impl<'a> IntoIterator for &'a ActionComboRouteSheet {
    type Item = (u32, Vec<(u16, ActionComboRouteRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ActionComboRouteRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> ActionComboRouteRow<'a> {
    pub fn Name(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn Action(&'a self) -> [&'a Field; 7] {
        [
