...
impl ActionComboRouteSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "ActionComboRoute")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "ActionComboRoute", &exh, language, i)?,
                );
        }
...
impl ActionComboRouteRow {
    ...
    pub fn Action<'a>(&'a self) -> [&'a ColumnData; 7] {
        [
            &self.columns[1],
            &self.columns[2],
            &self.columns[3],
