...
impl RecipeNotebookListSheet {
    ...
    pub fn read_from(
        ...
        let exh = resolver.read_excel_sheet_header("RecipeNotebookList")?;
        let sheet = resolver.read_excel_sheet(&exh, "RecipeNotebookList", language)?;
        Ok(Self { sheet })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn row(&self, row_id: u32) -> Option<RecipeNotebookListRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<RecipeNotebookListRow> {
...
impl<'a> StructuredSheet<'a> for RecipeNotebookListSheet {
    type Row = RecipeNotebookListRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a RecipeNotebookListSheet {
    type Item = (u32, Vec<(u16, RecipeNotebookListRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, RecipeNotebookListSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, RecipeNotebookListSheet> {
...
#[derive(Debug, Clone)]
pub struct RecipeNotebookListRow<'a> {
    row: &'a Row,
}
impl<'a> RecipeNotebookListRow<'a> {
    pub fn Recipe(&'a self) -> [i32; 160] {
        [
            ...
            self.row.columns[101].into_i32().copied().unwrap(),
            self.row.columns[102].into_i32().copied().unwrap(),
            self.row.columns[103].into_i32().copied().unwrap(),
            self.row.columns[104].into_i32().copied().unwrap(),
            self.row.columns[105].into_i32().copied().unwrap(),
            self.row.columns[106].into_i32().copied().unwrap(),
            self.row.columns[107].into_i32().copied().unwrap(),
            self.row.columns[108].into_i32().copied().unwrap(),
            self.row.columns[109].into_i32().copied().unwrap(),
            self.row.columns[110].into_i32().copied().unwrap(),
            self.row.columns[111].into_i32().copied().unwrap(),
            self.row.columns[112].into_i32().copied().unwrap(),
            self.row.columns[113].into_i32().copied().unwrap(),
            self.row.columns[114].into_i32().copied().unwrap(),
            self.row.columns[115].into_i32().copied().unwrap(),
            self.row.columns[116].into_i32().copied().unwrap(),
            self.row.columns[117].into_i32().copied().unwrap(),
            self.row.columns[118].into_i32().copied().unwrap(),
            self.row.columns[119].into_i32().copied().unwrap(),
            self.row.columns[120].into_i32().copied().unwrap(),
