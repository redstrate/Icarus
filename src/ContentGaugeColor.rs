...
impl<'a> StructuredSheet<'a> for ContentGaugeColorSheet {
    type Row = ContentGaugeColorRow<'a>;
...
impl<'a> IntoIterator for &'a ContentGaugeColorSheet {
    type Item = (u32, Vec<(u16, ContentGaugeColorRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ContentGaugeColorRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> ContentGaugeColorRow<'a> {
    pub fn AndroidColor1(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn AndroidColor2(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn AndroidColor3(&'a self) -> &'a Field {
