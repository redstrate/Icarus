...
impl<'a> StructuredSheet<'a> for HWDAnnounceSheet {
    type Row = HWDAnnounceRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a HWDAnnounceSheet {
    type Item = (u32, Vec<(u16, HWDAnnounceRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct HWDAnnounceRow<'a> {
    row: &'a Row,
}
impl<'a> HWDAnnounceRow<'a> {
