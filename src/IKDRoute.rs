...
impl<'a> StructuredSheet<'a> for IKDRouteSheet {
    type Row = IKDRouteRow<'a>;
...
impl<'a> IntoIterator for &'a IKDRouteSheet {
    type Item = (u32, Vec<(u16, IKDRouteRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct IKDRouteRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> IKDRouteRow<'a> {
    pub fn Name(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn Spot(&'a self) -> [&'a Field; 3] {
        [
