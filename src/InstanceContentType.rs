...
impl<'a> StructuredSheet<'a> for InstanceContentTypeSheet {
    type Row = InstanceContentTypeRow<'a>;
...
impl<'a> IntoIterator for &'a InstanceContentTypeSheet {
    type Item = (u32, Vec<(u16, InstanceContentTypeRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct InstanceContentTypeRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> InstanceContentTypeRow<'a> {
    pub fn StartLogMessage(&'a self) -> &'a Field {
    ...
    pub fn SupportsPartyMemberPortraits(&'a self) -> &'a Field {
