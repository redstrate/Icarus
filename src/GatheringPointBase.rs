...
impl<'a> StructuredSheet<'a> for GatheringPointBaseSheet {
    type Row = GatheringPointBaseRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a GatheringPointBaseSheet {
    type Item = (u32, Vec<(u16, GatheringPointBaseRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct GatheringPointBaseRow<'a> {
    row: &'a Row,
}
impl<'a> GatheringPointBaseRow<'a> {
    pub fn GatheringType(&'a self) -> i32 {
