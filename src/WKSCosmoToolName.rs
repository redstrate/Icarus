...
impl<'a> StructuredSheet<'a> for WKSCosmoToolNameSheet {
    type Row = WKSCosmoToolNameRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a WKSCosmoToolNameSheet {
    type Item = (u32, Vec<(u16, WKSCosmoToolNameRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct WKSCosmoToolNameRow<'a> {
    row: &'a Row,
}
impl<'a> WKSCosmoToolNameRow<'a> {
