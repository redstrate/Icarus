...
impl<'a> StructuredSheet<'a> for HousingRenovationSheet {
    type Row = HousingRenovationRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a HousingRenovationSheet {
    type Item = (u32, Vec<(u16, HousingRenovationRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct HousingRenovationRow<'a> {
    row: &'a Row,
}
impl<'a> HousingRenovationRow<'a> {
