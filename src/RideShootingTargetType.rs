...
impl<'a> StructuredSheet<'a> for RideShootingTargetTypeSheet {
    type Row = RideShootingTargetTypeRow<'a>;
...
impl<'a> IntoIterator for &'a RideShootingTargetTypeSheet {
    type Item = (u32, Vec<(u16, RideShootingTargetTypeRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct RideShootingTargetTypeRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> RideShootingTargetTypeRow<'a> {
    pub fn EObj(&'a self) -> &'a Field {
