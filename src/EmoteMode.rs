...
impl EmoteModeRow {
    pub fn StartEmote<'a>(&'a self) -> &'a Field {
        &self.columns[0]
    }
    pub fn EndEmote<'a>(&'a self) -> &'a Field {
        &self.columns[1]
    }
    pub fn ConditionMode<'a>(&'a self) -> &'a Field {
        &self.columns[2]
    }
    pub fn Move<'a>(&'a self) -> &'a Field {
        &self.columns[3]
    }
    pub fn Camera<'a>(&'a self) -> &'a Field {
        &self.columns[4]
    }
    pub fn EndOnRotate<'a>(&'a self) -> &'a Field {
        &self.columns[5]
    }
    pub fn EndOnEmote<'a>(&'a self) -> &'a Field {
        &self.columns[6]
    }
