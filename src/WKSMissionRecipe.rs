...
impl<'a> StructuredSheet<'a> for WKSMissionRecipeSheet {
    type Row = WKSMissionRecipeRow<'a>;
...
impl<'a> IntoIterator for &'a WKSMissionRecipeSheet {
    type Item = (u32, Vec<(u16, WKSMissionRecipeRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct WKSMissionRecipeRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> WKSMissionRecipeRow<'a> {
    pub fn Recipe(&'a self) -> [&'a Field; 5] {
        [
