...
impl WKSTerritoryInfoSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "WKSTerritoryInfo")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(
                    read_excel_sheet(resource, "WKSTerritoryInfo", &exh, language, i)?,
                );
        }
...
impl WKSTerritoryInfoRow {
    ...
    pub fn MechaEventNpc<'a>(&'a self) -> &'a ColumnData {
        &self.columns[8]
    }
