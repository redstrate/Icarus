...
impl<'a> StructuredSheet<'a> for TofuPresetObjectSheet {
    type Row = TofuPresetObjectRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a TofuPresetObjectSheet {
    type Item = (u32, Vec<(u16, TofuPresetObjectRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct TofuPresetObjectRow<'a> {
    row: &'a Row,
}
impl<'a> TofuPresetObjectRow<'a> {
    pub fn Object(&'a self) -> i32 {
