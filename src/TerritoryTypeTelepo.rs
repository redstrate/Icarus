...
impl<'a> StructuredSheet<'a> for TerritoryTypeTelepoSheet {
    type Row = TerritoryTypeTelepoRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a TerritoryTypeTelepoSheet {
    type Item = (u32, Vec<(u16, TerritoryTypeTelepoRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct TerritoryTypeTelepoRow<'a> {
    row: &'a Row,
}
impl<'a> TerritoryTypeTelepoRow<'a> {
    pub fn X(&'a self) -> u16 {
    ...
    pub fn Expansion(&'a self) -> u16 {
        self.row.columns[2].into_u16().copied().unwrap()
    }
    pub fn Relay(&'a self) -> u8 {
