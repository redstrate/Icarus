...
impl<'a> StructuredSheet<'a> for SatisfactionArbitrationSheet {
    type Row = SatisfactionArbitrationRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a SatisfactionArbitrationSheet {
    type Item = (u32, Vec<(u16, SatisfactionArbitrationRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct SatisfactionArbitrationRow<'a> {
    row: &'a Row,
}
impl<'a> SatisfactionArbitrationRow<'a> {
