...
impl<'a> StructuredSheet<'a> for ManeuversArmorSheet {
    type Row = ManeuversArmorRow<'a>;
...
impl<'a> IntoIterator for &'a ManeuversArmorSheet {
    type Item = (u32, Vec<(u16, ManeuversArmorRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ManeuversArmorRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> ManeuversArmorRow<'a> {
    ...
    pub fn FalconName(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn RavenName(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn NeutralMapIcon(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[4]]
    }
    pub fn FalconImage(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[5]]
    }
    pub fn RavenImage(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[6]]
    }
    pub fn FalconMapImage(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[7]]
    }
    pub fn RavenMapImage(&'a self) -> &'a Field {
