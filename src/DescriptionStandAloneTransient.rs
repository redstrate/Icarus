...
impl<'a> StructuredSheet<'a> for DescriptionStandAloneTransientSheet {
    type Row = DescriptionStandAloneTransientRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a DescriptionStandAloneTransientSheet {
    type Item = (u32, Vec<(u16, DescriptionStandAloneTransientRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct DescriptionStandAloneTransientRow<'a> {
    row: &'a Row,
}
impl<'a> DescriptionStandAloneTransientRow<'a> {
