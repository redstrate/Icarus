...
impl FishParameterSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "FishParameter")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "FishParameter", &exh, language, i)?);
        }
...
impl FishParameterRow {
    ...
    pub fn AchievementCredit<'a>(&'a self) -> &'a ColumnData {
        &self.columns[4]
    }
    pub fn Item<'a>(&'a self) -> &'a ColumnData {
        &self.columns[5]
    }
    pub fn GatheringItemLevel<'a>(&'a self) -> &'a ColumnData {
        &self.columns[6]
    }
    ...
    pub fn GatheringSubCategory<'a>(&'a self) -> &'a ColumnData {
        &self.columns[9]
    }
    pub fn OceanStars<'a>(&'a self) -> &'a ColumnData {
        &self.columns[10]
    }
    pub fn FishingRecordType<'a>(&'a self) -> &'a ColumnData {
        &self.columns[11]
    }
    pub fn IsHidden<'a>(&'a self) -> &'a ColumnData {
        &self.columns[12]
    }
    pub fn IsInLog<'a>(&'a self) -> &'a ColumnData {
        &self.columns[13]
    }
}
