...
impl DawnContentSheet {
    ...
    pub fn row(&self, row_id: u32) -> Option<DawnContentRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<DawnContentRow> {
...
impl<'a> StructuredSheet<'a> for DawnContentSheet {
    type Row = DawnContentRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a DawnContentSheet {
    type Item = (u32, Vec<(u16, DawnContentRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, DawnContentSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, DawnContentSheet> {
...
#[derive(Debug, Clone)]
pub struct DawnContentRow<'a> {
    row: &'a Row,
}
impl<'a> DawnContentRow<'a> {
    pub fn Content(&'a self) -> u32 {
        self.row.columns[0].into_u32().copied().unwrap()
    }
    pub fn ExpBelowExMaxLvl(&'a self) -> u32 {
        self.row.columns[6].into_u32().copied().unwrap()
    }
    pub fn ExpAboveExMaxLvl(&'a self) -> u32 {
