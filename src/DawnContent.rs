...
impl DawnContentSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "DawnContent")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "DawnContent", &exh, language, i)?);
        }
...
impl DawnContentRow {
    ...
    pub fn ExpBelowExMaxLvl<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn ExpAboveExMaxLvl<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
