...
impl<'a> StructuredSheet<'a> for GroupPoseFrameSheet {
    type Row = GroupPoseFrameRow<'a>;
...
impl<'a> IntoIterator for &'a GroupPoseFrameSheet {
    type Item = (u32, Vec<(u16, GroupPoseFrameRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct GroupPoseFrameRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> GroupPoseFrameRow<'a> {
    pub fn Text(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn GridText(&'a self) -> &'a Field {
