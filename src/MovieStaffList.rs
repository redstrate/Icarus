...
impl<'a> StructuredSheet<'a> for MovieStaffListSheet {
    type Row = MovieStaffListRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a MovieStaffListSheet {
    type Item = (u32, Vec<(u16, MovieStaffListRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MovieStaffListRow<'a> {
    row: &'a Row,
}
impl<'a> MovieStaffListRow<'a> {
