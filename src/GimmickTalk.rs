...
impl<'a> StructuredSheet<'a> for GimmickTalkSheet {
    type Row = GimmickTalkRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a GimmickTalkSheet {
    type Item = (u32, Vec<(u16, GimmickTalkRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct GimmickTalkRow<'a> {
    row: &'a Row,
}
impl<'a> GimmickTalkRow<'a> {
