...
impl WKSDevGradeSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "WKSDevGrade")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "WKSDevGrade", &exh, language, i)?);
        }
...
impl WKSDevGradeRow {
    pub fn StageFull<'a>(&'a self) -> &'a ColumnData {
        &self.columns[0]
    }
    pub fn Stage<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn IndexText2<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    pub fn IndexText<'a>(&'a self) -> &'a ColumnData {
        &self.columns[3]
    }
    pub fn IndexNpc<'a>(&'a self) -> &'a ColumnData {
        &self.columns[4]
    }
