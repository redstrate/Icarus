...
impl StructuredSheet for ActionTimelineReplaceSheet {
    type Row = ActionTimelineReplaceRow;
...
impl<'a> IntoIterator for &'a ActionTimelineReplaceSheet {
    type Item = (u32, Vec<(u16, ActionTimelineReplaceRow)>);
...
impl ActionTimelineReplaceRow {
    pub fn Old<'a>(&'a self) -> &'a Field {
        &self.columns[0]
    }
    pub fn New<'a>(&'a self) -> &'a Field {
        &self.columns[1]
    }
}
