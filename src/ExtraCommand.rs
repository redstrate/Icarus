...
impl<'a> StructuredSheet<'a> for ExtraCommandSheet {
    type Row = ExtraCommandRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a ExtraCommandSheet {
    type Item = (u32, Vec<(u16, ExtraCommandRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ExtraCommandRow<'a> {
    row: &'a Row,
}
impl<'a> ExtraCommandRow<'a> {
