...
impl<'a> BGMSceneRow<'a> {
    pub fn EnableDisableRestart(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn Resume(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn EnablePassEnd(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn ForceAutoReset(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn IgnoreBattle(&'a self) -> &'a Field {
