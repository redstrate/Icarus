...
impl StructuredSheet for BGMSceneSheet {
    type Row = BGMSceneRow;
...
impl<'a> IntoIterator for &'a BGMSceneSheet {
    type Item = (u32, Vec<(u16, BGMSceneRow)>);
...
impl BGMSceneRow {
    pub fn EnableDisableRestart<'a>(&'a self) -> &'a Field {
        &self.columns[0]
    }
    pub fn Resume<'a>(&'a self) -> &'a Field {
        &self.columns[1]
    }
    pub fn EnablePassEnd<'a>(&'a self) -> &'a Field {
        &self.columns[2]
    }
    pub fn ForceAutoReset<'a>(&'a self) -> &'a Field {
        &self.columns[3]
    }
    pub fn IgnoreBattle<'a>(&'a self) -> &'a Field {
        &self.columns[4]
    }
}
