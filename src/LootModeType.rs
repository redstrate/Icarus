...
impl<'a> StructuredSheet<'a> for LootModeTypeSheet {
    type Row = LootModeTypeRow<'a>;
...
impl<'a> IntoIterator for &'a LootModeTypeSheet {
    type Item = (u32, Vec<(u16, LootModeTypeRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct LootModeTypeRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> LootModeTypeRow<'a> {
    pub fn AdditionalLootRulesAvailable(&'a self) -> &'a Field {
