...
impl<'a> StructuredSheet<'a> for StatusLoopVFXSheet {
    type Row = StatusLoopVFXRow<'a>;
...
impl<'a> IntoIterator for &'a StatusLoopVFXSheet {
    type Item = (u32, Vec<(u16, StatusLoopVFXRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct StatusLoopVFXRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> StatusLoopVFXRow<'a> {
    pub fn VFX(&'a self) -> [&'a Field; 4] {
        [
