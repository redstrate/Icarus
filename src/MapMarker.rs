...
impl MapMarkerSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "MapMarker")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "MapMarker", &exh, language, i)?);
        }
...
impl MapMarkerRow {
    ...
    pub fn PlaceNameSubtext<'a>(&'a self) -> &'a ColumnData {
        &self.columns[1]
    }
    pub fn DataKey<'a>(&'a self) -> &'a ColumnData {
        &self.columns[2]
    }
    ...
    pub fn SubtextOrientation<'a>(&'a self) -> &'a ColumnData {
        &self.columns[5]
    }
    pub fn MapMarkerRegion<'a>(&'a self) -> &'a ColumnData {
        &self.columns[6]
    }
    pub fn Type<'a>(&'a self) -> &'a ColumnData {
        &self.columns[7]
    }
    pub fn DataType<'a>(&'a self) -> &'a ColumnData {
        &self.columns[8]
    }
