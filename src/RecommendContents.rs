...
impl<'a> StructuredSheet<'a> for RecommendContentsSheet {
    type Row = RecommendContentsRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a RecommendContentsSheet {
    type Item = (u32, Vec<(u16, RecommendContentsRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct RecommendContentsRow<'a> {
    row: &'a Row,
}
impl<'a> RecommendContentsRow<'a> {
