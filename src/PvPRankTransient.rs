...
impl<'a> StructuredSheet<'a> for PvPRankTransientSheet {
    type Row = PvPRankTransientRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a PvPRankTransientSheet {
    type Item = (u32, Vec<(u16, PvPRankTransientRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct PvPRankTransientRow<'a> {
    row: &'a Row,
}
impl<'a> PvPRankTransientRow<'a> {
