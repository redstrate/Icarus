...
impl<'a> StructuredSheet<'a> for CompanyActionSheet {
    type Row = CompanyActionRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a CompanyActionSheet {
    type Item = (u32, Vec<(u16, CompanyActionRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct CompanyActionRow<'a> {
    row: &'a Row,
}
impl<'a> CompanyActionRow<'a> {
