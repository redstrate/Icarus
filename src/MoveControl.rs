...
impl<'a> StructuredSheet<'a> for MoveControlSheet {
    type Row = MoveControlRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a MoveControlSheet {
    type Item = (u32, Vec<(u16, MoveControlRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct MoveControlRow<'a> {
    row: &'a Row,
}
impl<'a> MoveControlRow<'a> {
