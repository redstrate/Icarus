...
impl ContentRouletteSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        ...
        for (i, _) in exh.pages.iter().enumerate() {
            pages
                .push(read_excel_sheet(resource, "ContentRoulette", &exh, language, i)?);
        }
...
impl ContentRouletteRow {
    ...
    pub fn ContentRouletteOpenRule<'a>(&'a self) -> &'a ColumnData {
        &self.columns[30]
    }
    ...
    pub fn RequireAllDuties<'a>(&'a self) -> &'a ColumnData {
        &self.columns[42]
    }
