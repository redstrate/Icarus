...
impl<'a> EventIconTypeRow<'a> {
    pub fn NpcIconAvailable(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn MapIconAvailable(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[1]]
    }
    pub fn NpcIconInvalid(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[2]]
    }
    pub fn MapIconInvalid(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[3]]
    }
    pub fn IconRange(&'a self) -> &'a Field {
