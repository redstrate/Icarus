...
impl FollowMountSheet {
    ...
    pub fn read_from(
        ...
        let exh = resolver.read_excel_sheet_header("FollowMount")?;
        let sheet = resolver.read_excel_sheet(&exh, "FollowMount", language)?;
        Ok(Self { sheet })
    }
    /// Fetches a single row from the sheet. If the row contains subrows, it returns the first one.
    pub fn row(&self, row_id: u32) -> Option<FollowMountRow> {
        let row = &self.sheet.row(row_id)?;
        self.read_row(row)
    }
    /// Fetches the specified subrow from the sheet.
    pub fn subrow(&self, row_id: u32, subrow_id: u16) -> Option<FollowMountRow> {
...
impl<'a> StructuredSheet<'a> for FollowMountSheet {
    type Row = FollowMountRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a FollowMountSheet {
    type Item = (u32, Vec<(u16, FollowMountRow<'a>)>);
    type IntoIter = StructuredSheetIterator<'a, FollowMountSheet>;
    fn into_iter(self) -> StructuredSheetIterator<'a, FollowMountSheet> {
...
#[derive(Debug, Clone)]
pub struct FollowMountRow<'a> {
    row: &'a Row,
}
impl<'a> FollowMountRow<'a> {
