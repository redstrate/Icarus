...
impl<'a> StructuredSheet<'a> for WeaponTimelineSheet {
    type Row = WeaponTimelineRow<'a>;
...
impl<'a> IntoIterator for &'a WeaponTimelineSheet {
    type Item = (u32, Vec<(u16, WeaponTimelineRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct WeaponTimelineRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> WeaponTimelineRow<'a> {
    pub fn File(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn NextWeaponTimeline(&'a self) -> &'a Field {
