...
impl<'a> StructuredSheet<'a> for BannerDecorationSheet {
    type Row = BannerDecorationRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a BannerDecorationSheet {
    type Item = (u32, Vec<(u16, BannerDecorationRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct BannerDecorationRow<'a> {
    row: &'a Row,
}
impl<'a> BannerDecorationRow<'a> {
