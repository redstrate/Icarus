...
impl<'a> StructuredSheet<'a> for PublicContentCutsceneSheet {
    type Row = PublicContentCutsceneRow<'a>;
...
impl<'a> IntoIterator for &'a PublicContentCutsceneSheet {
    type Item = (u32, Vec<(u16, PublicContentCutsceneRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct PublicContentCutsceneRow<'a> {
    row: &'a Row,
    index_mapping: Vec<usize>,
}
impl<'a> PublicContentCutsceneRow<'a> {
    pub fn Cutscene(&'a self) -> &'a Field {
        &self.row.columns[self.index_mapping[0]]
    }
    pub fn Cutscene2(&'a self) -> &'a Field {
