...
impl GatheringRarePopTimeTableSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "GatheringRarePopTimeTable")?;
    ...
    fn read_row(&self, row: &ExcelSingleRow) -> Option<GatheringRarePopTimeTableRow> {
        ...
        let (columns, _): (Vec<ColumnData>, Vec<ExcelColumnDefinition>) = zipped
            .into_iter()
            .unzip();
        Some(GatheringRarePopTimeTableRow {
            columns,
        })
    }
...
impl GatheringRarePopTimeTableRow {
    pub fn StartTime<'a>(&'a self) -> [&'a ColumnData; 3] {
        [&self.columns[0], &self.columns[1], &self.columns[2]]
    }
    pub fn Duration<'a>(&'a self) -> [&'a ColumnData; 3] {
