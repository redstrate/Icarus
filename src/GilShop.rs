...
impl GilShopSheet {
    ...
    pub fn read_from<T: Resource>(resource: &mut T, language: Language) -> Option<Self> {
        let exh = read_excel_sheet_header(resource, "GilShop")?;
        let mut pages = Vec::new();
        for (i, _) in exh.pages.iter().enumerate() {
            pages.push(read_excel_sheet(resource, "GilShop", &exh, language, i)?);
        }
...
impl GilShopRow {
    ...
    pub fn AcceptTalk<'a>(&'a self) -> &'a ColumnData {
        &self.columns[3]
    }
    pub fn FailTalk<'a>(&'a self) -> &'a ColumnData {
        &self.columns[4]
    }
    pub fn FestivalId<'a>(&'a self) -> &'a ColumnData {
        &self.columns[5]
    }
    pub fn FestivalPhase<'a>(&'a self) -> &'a ColumnData {
        &self.columns[6]
    }
