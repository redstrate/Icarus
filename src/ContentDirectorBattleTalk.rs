...
impl<'a> StructuredSheet<'a> for ContentDirectorBattleTalkSheet {
    type Row = ContentDirectorBattleTalkRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a ContentDirectorBattleTalkSheet {
    type Item = (u32, Vec<(u16, ContentDirectorBattleTalkRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ContentDirectorBattleTalkRow<'a> {
    row: &'a Row,
}
impl<'a> ContentDirectorBattleTalkRow<'a> {
