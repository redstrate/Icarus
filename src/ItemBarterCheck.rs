...
impl<'a> StructuredSheet<'a> for ItemBarterCheckSheet {
    type Row = ItemBarterCheckRow<'a>;
    fn read_row(&self, row: &'a Row) -> Option<Self::Row> {
        Some(Self::Row { row })
    }
}
impl<'a> IntoIterator for &'a ItemBarterCheckSheet {
    type Item = (u32, Vec<(u16, ItemBarterCheckRow<'a>)>);
...
#[derive(Debug, Clone)]
pub struct ItemBarterCheckRow<'a> {
    row: &'a Row,
}
impl<'a> ItemBarterCheckRow<'a> {
    pub fn Question(&'a self) -> u32 {
        self.row.columns[1].into_u32().copied().unwrap()
    }
    pub fn Confirm(&'a self) -> u32 {
